...
void test_new_parser_events_ints_from_yaml(ReferenceYaml const& yaml, std::string const& expected_events)
{
    ...
    actual_evts.resize(sz);
    #ifdef RYML_DBG
    extra::print_events_ints(to_csubstr(copy), to_csubstr(arena), actual_evts.data(), (IntType)actual_evts.size());
    #endif
    {
    ...
    }
    {
        ...
        std::string actual_test_suite_evts = extra::emit_events_test_suite_from_ints<std::string>(to_csubstr(copy), to_csubstr(arena), actual_evts.data(), (IntType)actual_evts.size());
