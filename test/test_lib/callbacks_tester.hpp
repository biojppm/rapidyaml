...
struct CallbacksTester
{
    ...
    void check()
    {
        ...
        RYML_CHECK_BASIC_(num_allocs == num_deallocs);
        RYML_CHECK_BASIC_(alloc_size == dealloc_size);
    }
    ...
    void *allocate(size_t len)
    {
        ...
        alloc_size += len;
        ++num_allocs;
        RYML_CHECK_BASIC_(alloc_size < memory_pool.size());
        return ptr;
    }

    void free(void *mem, size_t len)
    {
        RYML_CHECK_BASIC_((char*)mem     >= &memory_pool.front() && (char*)mem     <  &memory_pool.back());
        RYML_CHECK_BASIC_((char*)mem+len >= &memory_pool.front() && (char*)mem+len <= &memory_pool.back());
