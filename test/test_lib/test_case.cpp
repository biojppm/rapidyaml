...
extern void report_error_parse_impl(const char* msg, size_t len, Location loc, FILE *file);
#endif

std::string format_error_basic(const char* msg, size_t len, Location const& cpploc)
{
    std::string out;
    err_basic_print(msg, len, cpploc, nullptr); // ensure coverage of the default callback report
    err_basic_fmt(msg, len, cpploc, [&](csubstr s){
        out.append(s.str, s.len);
    });
    return out;
}

std::string format_error_parse(const char* msg, size_t len, Location const& cpploc, Location const& ymlloc)
{
    std::string out;
    err_parse_print(msg, len, cpploc, ymlloc, nullptr); // ensure coverage of the default callback report
    err_parse_fmt(msg, len, cpploc, ymlloc, [&](csubstr s){
        out.append(s.str, s.len);
    });
    return out;
}

std::string format_error_visit(const char* msg, size_t len, Location const& cpploc, Tree const* t, id_type id)
{
    std::string out;
    err_visit_print(msg, len, cpploc, t, id, nullptr); // ensure coverage of the default callback report
    err_visit_fmt(msg, len, cpploc, t, id, [&](csubstr s){
        out.append(s.str, s.len);
    });
    return out;
}
...
struct ExpectedErrorBasic C4_IF_EXCEPTIONS_( : public std::exception, )
{
    ...
    ExpectedErrorBasic(const char* msg, size_t len, Location const& cpploc_)
        :
        C4_IF_EXCEPTIONS_(std::exception() C4_COMMA,)
        smsg(format_error_basic(msg, len, cpploc_))
    {
    }
...
struct ExpectedErrorParse : public ExpectedErrorBasic
{
    ...
    ExpectedErrorParse(const char* msg, size_t len, Location const& cpploc_, Location const& ymlloc_)
        : ExpectedErrorBasic(format_error_parse(msg, len, cpploc_, ymlloc_))
        , ymlloc(ymlloc_)
    {
    }
};
...
struct ExpectedErrorVisit : public ExpectedErrorBasic
{
    ...
    ExpectedErrorVisit(const char* msg, size_t len, Location const& cpploc_, Tree const* t, id_type id_)
        : ExpectedErrorBasic(format_error_visit(msg, len, cpploc_, t, id_))
        , tree(t)
        , id(id_)
    {
    }
...
{
    auto errb = [](const char* msg, size_t len, Location const& cpploc, void *this_) {
        ...
        C4_IF_EXCEPTIONS(
            throw ExpectedErrorBasic(msg, len, cpploc);
            ,
            s_jmp_err_basic = ExpectedErrorBasic(msg, len, cpploc);
            std::longjmp(s_jmp_env_expect_error, 1);
        );
        C4_UNREACHABLE_AFTER_ERR();
    };
    auto errp = [](const char* msg, size_t len, Location const& cpploc, Location const& ymlloc, void *this_) {
        _c4dbgpf("called parse error callback! (withlocation={})", bool(ymlloc));
//EXPECT_TRUE(ymlloc) << ymlloc;
        ((ExpectError*)this_)->m_got_an_error = true; // assign in here to ensure the exception was thrown here
        C4_IF_EXCEPTIONS(
            throw ExpectedErrorParse(msg, len, cpploc, ymlloc);
            ,
            s_jmp_err_parse = ExpectedErrorParse(msg, len, cpploc, ymlloc);
            std::longjmp(s_jmp_env_expect_error, 1);
        );
        C4_UNREACHABLE_AFTER_ERR();
    };
    auto errv = [](const char* msg, size_t len, Location const& cpploc, Tree const* t, id_type id, void *this_) {
        _c4dbgp("called visit error callback!");
        ((ExpectError*)this_)->m_got_an_error = true; // assign in here to ensure the exception was thrown here
        C4_IF_EXCEPTIONS(
            throw ExpectedErrorVisit(msg, len, cpploc, t, id);
            ,
            s_jmp_err_visit = ExpectedErrorVisit(msg, len, cpploc, t, id);
            std::longjmp(s_jmp_env_expect_error, 1);
        );
        C4_UNREACHABLE_AFTER_ERR();
    };
...
void ExpectError::check_error_parse(Tree *tree, fntestref fn, Location const& expected_location)
{
    ...
    C4_IF_EXCEPTIONS_(catch(ExpectedErrorParse const& e), else)
    {
        ...
        if(expected_location)
        {
            ...
            EXPECT_EQ(static_cast<bool>(e.ymlloc), static_cast<bool>(context.expected_location));
            EXPECT_EQ(e.ymlloc.line, context.expected_location.line);
            EXPECT_EQ(e.ymlloc.col, context.expected_location.col);
            if(context.expected_location.offset)
            {
                EXPECT_EQ(e.ymlloc.offset, context.expected_location.offset);
            }
        }
    }
    C4_IF_EXCEPTIONS_(catch(...)
