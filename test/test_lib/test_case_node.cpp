...
//-----------------------------------------------------------------------------
void TestCaseNode::compare_child(yml::ConstNodeRef const& n, size_t pos) const
{
    SCOPED_TRACE(pos);
    SCOPED_TRACE(n.id());
...
void TestCaseNode::compare(yml::ConstNodeRef const& actual, bool ignore_quote) const
{
    ...
    if(ignore_quote)
    {
        const type_bits mask = VALQUO | KEYQUO | _WIP_KEY_STYLE | _WIP_VAL_STYLE | _WIP_CONTAINER_STYLE;
