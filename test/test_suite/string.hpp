...
public:
    ...
    void _free()
    {
        RYML_ASSERT_BASIC(m_str != nullptr); // this structure cannot be memset() to zero
        if(m_str != m_buf)
        {
        ...
        RYML_ASSERT_BASIC(m_capacity == sso_size);
        m_size = 0;
    }
    ...
    void _cp(string const* C4_RESTRICT that)
    {
        ...
        {
            RYML_ASSERT_BASIC(that->m_capacity > sso_size);
            RYML_ASSERT_BASIC(that->m_size <= that->m_capacity);
        }
        else
        {
            RYML_ASSERT_BASIC(that->m_capacity <= sso_size);
    ...
    void _mv(string *C4_RESTRICT that)
    {
        ...
        RYML_ASSERT_BASIC(that->m_str != this->m_buf);
...
public:
    ...
    void _free()
    {
        RYML_ASSERT_BASIC(m_arr != nullptr); // this structure cannot be memset() to zero
...
public:
    ...
    string& emplace_back()
    {
        RYML_ASSERT_BASIC(m_size < m_capacity);
