...
struct Scalar
{
    ...
    void add_key_props(Tree *tree, size_t node) const
    {
        ...
        if(flags != NOTYPE)
        {
            ...
            tree->_add_flags(node, flags & _WIP_KEY_STYLE);
        }
    }
    void add_val_props(Tree *tree, size_t node) const
    {
        ...
        if(flags != NOTYPE)
        {
            ...
            tree->_add_flags(node, flags & _WIP_VAL_STYLE);
        }
    }
    ...
    for(csubstr line : src.split('\n'))
    {
        ...
        if(line.begins_with("=VAL "))
        {
            ...
            if(line.begins_with('"'))
            {
                ...
                curr.flags = _WIP_SCALAR_DQUO;
            }
            else if(line.begins_with('\''))
            {
                _nfo_llog("single-quoted scalar!");
                curr.scalar = line.sub(1);
                curr.flags = _WIP_SCALAR_SQUO;
            }
            else if(line.begins_with('|'))
            {
                _nfo_llog("block literal scalar!");
                curr.scalar = line.sub(1);
                curr.flags = _WIP_SCALAR_LITERAL;
            }
            else if(line.begins_with('>'))
            {
                _nfo_llog("block folded scalar!");
                curr.scalar = line.sub(1);
                curr.flags = _WIP_SCALAR_FOLDED;
            }
            else
            {
        ...
        else if(line.begins_with("+SEQ"))
        {
            ...
            if(more_tokens.begins_with('['))
            {
                ...
                more_flags.add(_WIP_STYLE_FLOW_SL);
                _nfo_log("seq is flow");
            }
            else
            {
                more_flags.add(_WIP_STYLE_BLOCK);
