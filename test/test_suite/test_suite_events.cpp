...
struct Scalar
{
    ...
    OptionalScalar tag    = {};
    NodeType       flags = {};
    inline operator bool() const { if(anchor || tag) { RYML_ASSERT_BASIC(scalar); } return scalar.was_set; }
...
csubstr parse_anchor_and_tag(csubstr tokens, OptionalScalar *anchor, OptionalScalar *tag)
{
    ...
    *tag = OptionalScalar{};
    ...
    if(tokens.begins_with('<'))
    {
        ...
        RYML_ASSERT_BASIC(pos != (size_t)csubstr::npos);
...
bool compare_events(csubstr ref_evts, csubstr emt_evts, bool ignore_container_style=false, bool ignore_scalar_style=false)
{
    ...
    auto diff_val_with_scalar_wildcard = [](csubstr ref, csubstr emt){
        RYML_ASSERT_BASIC(ref.begins_with("=VAL "));
        RYML_ASSERT_BASIC(emt.begins_with("=VAL "));
        ref = ref.sub(5);
        emt = emt.sub(5);
        ...
        RYML_ASSERT_BASIC(ref.len > 0);
        RYML_ASSERT_BASIC(emt.len > 0);
        RYML_ASSERT_BASIC(ref[0] == ':' || ref[0] == '\'' || ref[0] == '"' || ref[0] == '|' || ref[0] == '>');
        RYML_ASSERT_BASIC(emt[0] == ':' || emt[0] == '\'' || emt[0] == '"' || emt[0] == '|' || emt[0] == '>');
        ref = ref.sub(1);
        emt = emt.sub(1);
        if(ref != emt)
            return true;
