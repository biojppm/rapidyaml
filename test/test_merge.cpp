...
TEST(merge, dst_scalar_keeps_style)
{
    ...
    EXPECT_EQ(dst["a"], "b");
    EXPECT_EQ(src["a"], "c");
    EXPECT_TRUE(dst["a"].type().key_marked_plain());
    EXPECT_TRUE(dst["a"].type().val_marked_plain());
    EXPECT_TRUE(src["a"].type().key_marked_squo());
    EXPECT_TRUE(src["a"].type().val_marked_squo());
    _c4dbg_tree("src", src);
    _c4dbg_tree("dst", dst);
    dst.merge_with(&src);
