...
substr Emitter<Writer>::emit_as(EmitType_e type, Tree const& tree, id_type id, bool error_on_excess)
{
    if(tree.empty())
    {
        _RYML_CB_ASSERT_BASIC(tree.callbacks(), id == NONE);
        return {};
    }
    if(id == NONE)
        id = tree.root_id();
    _RYML_CB_CHECK_VISIT(tree.callbacks(), id < tree.capacity(), &tree, id);
    m_tree = &tree;
    m_flow = false;
    if(type == EMIT_YAML)
        _emit_yaml(id);
        ...
        _RYML_CB_ERR_BASIC(m_tree->callbacks(), "unknown emit type");
    m_tree = nullptr;
...
void Emitter<Writer>::_emit_yaml(id_type id)
{
    ...
    {
        _RYML_CB_ASSERT_VISIT(m_tree->callbacks(), !m_tree->is_container(id), m_tree, id); // checked above
        _RYML_CB_ASSERT_VISIT(m_tree->callbacks(), m_tree->is_val(id), m_tree, id); // so it must be a val
        _write_doc(id);
    }
    ...
    {
        _RYML_CB_ERR_VISIT(m_tree->callbacks(), m_tree, id, "unknown type");
    }
}
...
void Emitter<Writer>::_write_doc(id_type id)
{
    ...
    _RYML_CB_ASSERT_VISIT(m_tree->m_callbacks, ty.is_doc(), m_tree, id);
    _RYML_CB_ASSERT_VISIT(m_tree->m_callbacks, !ty.has_key(), m_tree, id);
    if(!m_tree->is_root(id))
    {
        _RYML_CB_ASSERT_VISIT(m_tree->m_callbacks, m_tree->is_stream(m_tree->parent(id)), m_tree, id);
        this->Writer::_do_write("---");
    }
    //
    ...
    {
        _RYML_CB_ASSERT_VISIT(m_tree->callbacks(), ty.has_val(), m_tree, id);
...
void Emitter<Writer>::_do_visit_flow_sl(id_type node, id_type depth, id_type ilevel)
{
    ...
    m_flow = true;
    _RYML_CB_ASSERT_VISIT(m_tree->callbacks(), !m_tree->is_stream(node), m_tree, node);
    _RYML_CB_ASSERT_VISIT(m_tree->callbacks(), m_tree->is_container(node) || m_tree->is_doc(node), m_tree, node);
    _RYML_CB_ASSERT_VISIT(m_tree->callbacks(), m_tree->is_root(node) || (m_tree->parent_is_map(node) || m_tree->parent_is_seq(node)), m_tree, node);
    if(C4_UNLIKELY(depth > m_opts.max_depth()))
        _RYML_CB_ERR_VISIT(m_tree->callbacks(), m_tree, node, "max depth exceeded");
    ...
    if(m_tree->is_doc(node))
    {
        ...
        #endif
        {
            ...
            {
                _RYML_CB_ASSERT_VISIT(m_tree->callbacks(), m_tree->is_seq(node), m_tree, node);
                this->Writer::_do_write('[');
            }
        }
    }
    ...
    {
        _RYML_CB_ASSERT_VISIT(m_tree->callbacks(), m_tree->is_map(node) || m_tree->is_seq(node), m_tree, node);
...
void Emitter<Writer>::_do_visit_block_container(id_type node, id_type depth, id_type level, bool do_indent)
{
    if(m_tree->is_seq(node))
    {
        ...
        {
            _RYML_CB_ASSERT_VISIT(m_tree->callbacks(), !m_tree->has_key(child), m_tree, node);
            ...
            {
                _RYML_CB_ASSERT_VISIT(m_tree->callbacks(), m_tree->is_container(child), m_tree, node);
    ...
    {
        _RYML_CB_ASSERT_VISIT(m_tree->callbacks(), m_tree->is_map(node), m_tree, node);
        for(id_type ich = m_tree->first_child(node); ich != NONE; ich = m_tree->next_sibling(ich))
        {
            _RYML_CB_ASSERT_VISIT(m_tree->callbacks(), m_tree->has_key(ich), m_tree, node);
            ...
            {
                _RYML_CB_ASSERT_VISIT(m_tree->callbacks(), m_tree->is_container(ich), m_tree, node);
...
{
    _RYML_CB_CHECK_VISIT(m_tree->callbacks(), !m_tree->is_stream(id), m_tree, id); // JSON does not have streams
    if(C4_UNLIKELY(depth > m_opts.max_depth()))
        _RYML_CB_ERR_VISIT(m_tree->callbacks(), m_tree, id, "max depth exceeded");
...
void Emitter<Writer>::_write(NodeScalar const& C4_RESTRICT sc, NodeType flags, id_type ilevel)
{
    ...
    if(flags.has_anchor())
    {
        _RYML_CB_ASSERT_VISIT(m_tree->callbacks(), flags.is_ref() != flags.has_anchor(), m_tree, NONE);
        _RYML_CB_ASSERT_VISIT(m_tree->callbacks(),  ! sc.anchor.empty(), m_tree, NONE);
        this->Writer::_do_write('&');
    ...
    _RYML_CB_ASSERT_VISIT(m_tree->callbacks(), ((flags & SCALAR_STYLE) == 0) || (((flags & KEY_STYLE) == 0) != ((flags & VAL_STYLE) == 0)), m_tree, NONE);
    ...
    {
        _RYML_CB_ERR_VISIT(m_tree->callbacks(), m_tree, NONE, "not implemented");
    }
}
...
void Emitter<Writer>::_write_json(NodeScalar const& C4_RESTRICT sc, NodeType flags)
{
    if(flags & (KEYTAG|VALTAG))
        if(m_opts.json_error_flags() & EmitOptions::JSON_ERR_ON_TAG)
            _RYML_CB_ERR_VISIT(m_tree->callbacks(), m_tree, NONE, "JSON does not have tags");
    if(C4_UNLIKELY(flags.has_anchor()))
        if(m_opts.json_error_flags() & EmitOptions::JSON_ERR_ON_ANCHOR)
            _RYML_CB_ERR_VISIT(m_tree->callbacks(), m_tree, NONE, "JSON does not have anchors");
    if(sc.scalar.len)
    {
...
{
    _RYML_CB_ASSERT_BASIC(m_tree->callbacks(), s.len > i);
    _RYML_CB_ASSERT_BASIC(m_tree->callbacks(), s.str[i] == '\n');
    ...
    _RYML_CB_ASSERT_BASIC(m_tree->callbacks(), i > 0);
    --i;
    _RYML_CB_ASSERT_BASIC(m_tree->callbacks(), s.str[i] == '\n');
    return i;
}
...
size_t Emitter<Writer>::_write_indented_block(csubstr s, size_t i, id_type ilevel)
{
    ...
    _RYML_CB_ASSERT_BASIC(m_tree->callbacks(), i > 0);
    _RYML_CB_ASSERT_BASIC(m_tree->callbacks(), s.str[i-1] == '\n');
    _RYML_CB_ASSERT_BASIC(m_tree->callbacks(), i < s.len);
    _RYML_CB_ASSERT_BASIC(m_tree->callbacks(), s.str[i] == ' ' || s.str[i] == '\t' || s.str[i] == '\n');
again:
    size_t pos = s.find("\n ", i);
    if(pos == npos)
        pos = s.find("\n\t", i);
...
{
    _RYML_CB_ASSERT_BASIC(m_tree->callbacks(), s.find("\r") == csubstr::npos);
    if(explicit_key)
        this->Writer::_do_write("? ");
