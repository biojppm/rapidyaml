...
void Emitter<Writer>::_flow_map_open_entry(id_type node)
{
    ...
    else
    {
        ...
        if(!(ty & _styles_flow_key))
            ty |= scalar_style_choose(key) & static_cast<NodeType_e>(_styles_flow_key);
        _flow_write_scalar(key, ty & _styles_flow_key);
    }
...
void Emitter<Writer>::_visit_flow_sl_seq(id_type node)
{
    ...
    for(id_type child = m_tree->first_child(node), last = m_tree->last_child(node); child != NONE; child = m_tree->next_sibling(child))
    {
        ...
        if(ty.is_val())
        {
            ...
            if(!ty.is_val_ref())
            {
                if(!(ty & _styles_flow_val))
                    ty |= (scalar_style_choose(val) & static_cast<NodeType_e>(_styles_flow_val));
                _flow_write_scalar(val, ty & _styles_flow_val);
            }
            else
            {
                _write_ref(val);
