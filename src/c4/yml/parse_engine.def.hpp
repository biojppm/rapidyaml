...
{
    RYML_ASSERT_BASIC(s.len > 0);
    RYML_ASSERT_BASIC(s.str[0] == '-' || s.str[0] == ':' || s.str[0] == '?');
    return ((s.len == 1) || ((s.str[1] == ' ') _RYML_WITH_TAB_TOKENS( || (s.str[1] == '\t'))));
}

inline bool _is_doc_begin_token(csubstr s)
{
    RYML_ASSERT_BASIC(s.begins_with('-'));
    RYML_ASSERT_BASIC(!s.ends_with("\n"));
    RYML_ASSERT_BASIC(!s.ends_with("\r"));
    ...
    , m_newline_offsets_buf()
{
    if(that.m_newline_offsets_capacity)
    {
        ...
        _RYML_CB_CHECK_BASIC(m_evt_handler->m_stack.m_callbacks, m_newline_offsets_capacity == that.m_newline_offsets_capacity);
...
ParseEngine<EventHandler>& ParseEngine<EventHandler>::operator=(ParseEngine const& that)
{
    if(&that != this)
    {
        ...
        if(that.m_newline_offsets_capacity > m_newline_offsets_capacity)
            _resize_locations(that.m_newline_offsets_capacity);
        _RYML_CB_CHECK_BASIC(m_evt_handler->m_stack.m_callbacks, m_newline_offsets_capacity >= that.m_newline_offsets_capacity);
        _RYML_CB_CHECK_BASIC(m_evt_handler->m_stack.m_callbacks, m_newline_offsets_capacity >= that.m_newline_offsets_size);
        memcpy(m_newline_offsets, that.m_newline_offsets, that.m_newline_offsets_size * sizeof(size_t));
        m_newline_offsets_size = that.m_newline_offsets_size;
        m_newline_offsets_buf = that.m_newline_offsets_buf;
    }
...
C4_NORETURN C4_NO_INLINE void ParseEngine<EventHandler>::_err(Location const& cpploc, Location const& ymlloc, const char* fmt, Args const& ...args) const
{
    ...
    err_parse_msg(m_evt_handler->m_stack.m_callbacks, cpploc, ymlloc, fmt, args...);
}
...
{
    m_evt_handler->cancel_parse();
    err_parse_msg(m_evt_handler->m_stack.m_callbacks, cpploc, m_evt_handler->m_curr->pos, fmt, args...);
}
...
{
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, m_evt_handler->m_curr->line_contents.rem.begins_with_any(chars));
...
{
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, m_evt_handler->m_curr->line_contents.rem.begins_with('#'));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, m_evt_handler->m_curr->line_contents.rem.is_sub(m_evt_handler->m_curr->line_contents.full));
    csubstr rem = m_evt_handler->m_curr->line_contents.rem;
    csubstr full = m_evt_handler->m_curr->line_contents.full;
    // raise an error if the comment is not preceded by whitespace
    if(!full.begins_with('#'))
    {
        _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, rem.str > full.str);
...
csubstr ParseEngine<EventHandler>::_scan_anchor()
{
    ...
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, s.begins_with('&'));
...
csubstr ParseEngine<EventHandler>::_scan_tag()
{
    ...
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, rem.begins_with('!'));
    csubstr t;
    if(rem.begins_with("!!"))
    {
        _c4dbgp("begins with '!!'");
...
{
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, !s.empty());
...
{
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_none(RMAP));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_none(BLCK));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_any(RSEQ|RSEQIMAP));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_any(FLOW));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_any(RVAL));
...
{
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_none(RSEQ) || has_any(RSEQIMAP));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_none(BLCK));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_any(RMAP|RSEQIMAP));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_any(FLOW));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_any(RKEY|RVAL|QMRK));
...
bool ParseEngine<EventHandler>::_scan_scalar_seq_json(ScannedScalar *C4_RESTRICT sc)
{
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_none(RMAP));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_none(BLCK));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_any(RSEQ));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_any(FLOW));
...
{
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_none(RSEQ));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_none(BLCK));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_any(RMAP));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_any(FLOW));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_any(RKEY|RVAL));
...
{
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, s[0] == '-');
...
{
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_none(FLOW));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_none(RSEQIMAP));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_any(BLCK|RUNK|USTY));
...
bool ParseEngine<EventHandler>::_scan_scalar_plain_seq_blck(ScannedScalar *C4_RESTRICT sc)
{
    ...
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_none(RSEQIMAP));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_any(RSEQ));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_any(BLCK));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_any(RVAL));
...
{
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks,  has_any(RUNK|USTY));
...
void ParseEngine<EventHandler>::_line_progressed(size_t ahead)
{
    ...
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, m_evt_handler->m_curr->pos.col <= m_evt_handler->m_curr->line_contents.stripped.len+1);
...
{
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, m_evt_handler->m_curr->pos.col == 1u);
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, m_evt_handler->m_curr->pos.line > 0u);
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, m_evt_handler->m_curr->pos.offset >= m_evt_handler->m_curr->line_contents.full.len - m_evt_handler->m_curr->line_contents.stripped.len);
    ...
    //_RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, m_evt_handler->m_curr->pos.offset >= m_buf.len || m_buf[m_evt_handler->m_curr->pos.offset] == '\n' || m_buf[m_evt_handler->m_curr->pos.offset] == '\r');
...
{
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, m_evt_handler->m_curr->line_contents.rem.begin() >= m_evt_handler->m_curr->line_contents.full.begin());
...
void ParseEngine<EventHandler>::_end2_map()
{
    ...
    {
        _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_none(FLOW));
        _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_any(USTY));
        m_evt_handler->_pop();
    }
}
...
{
    _c4dbgp("doc: end");
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_any(RDOC));
...
{
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, m_evt_handler->m_stack.size() >= 1);
...
void ParseEngine<EventHandler>::_handle_indentation_pop_from_block_seq()
{
    ...
    _RYML_CB_ASSERT_BASIC(stack.m_callbacks, stack.is_contiguous()); // this search relies on the stack being contiguous
    _RYML_CB_ASSERT_BASIC(stack.m_callbacks, m_evt_handler->m_curr >= stack.begin() && m_evt_handler->m_curr < stack.end());
    ...
    for(state_type const* s = m_evt_handler->m_curr-1; s > stack.begin(); --s) // never go to the stack bottom. that's the root
    {
        ...
        else if(s->indref == ind)
        {
            ...
            if(has_all(RSEQ|BLCK, s))
            {
                ...
                _RYML_CB_ASSERT_BASIC(stack.m_callbacks, first == ind || first == npos);
...
typename ParseEngine<EventHandler>::ScannedScalar ParseEngine<EventHandler>::_scan_scalar_squot()
{
    ...
    {
        s = s.triml(' ');
        _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, m_buf.sub(b).is_super(s));
        _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, s.begin() >= m_buf.sub(b).begin());
    ...
    while( ! _finished_file())
    {
        ...
        {
            _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, pos >= 0 && pos < m_buf.len);
            _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, m_buf[m_evt_handler->m_curr->pos.offset + pos] == '\'');
    ...
    {
        _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, pos > 0);
        _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, s.end() >= m_buf.begin() && s.end() <= m_buf.end());
        _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, s.end() == m_buf.end() || *s.end() == '\'');
        s = s.sub(0, pos-1);
    }
...
void ParseEngine<EventHandler>::_scan_block(ScannedBlock *C4_RESTRICT sb, size_t indref)
{
    ...
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, indref != npos);
    ...
    if(trimmed.str > s.str)
    {
        ...
        _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, trimmed.str >= s.str);
        _line_progressed(static_cast<size_t>(trimmed.str - s.str));
        s = trimmed;
    }
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, s.begins_with('|') || s.begins_with('>'));
    ...
    if(s.len > 1)
    {
        _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, s.begins_with_any("|>"));
        csubstr t = s.sub(1);
        _c4dbgpf("blck: spec is multichar: '{}'", t);
        _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, t.len >= 1);
    ...
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, raw_block.begin() == m_evt_handler->m_curr->line_contents.full.begin());
    ...
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, m_evt_handler->m_curr->pos.line == (first + num_lines) || (raw_block.len == 0));
    C4_UNUSED(num_lines);
    C4_UNUSED(first);
...
bool ParseEngine<EventHandler>::_filter_ws_handle_to_first_non_space(FilterProcessor &proc)
{
    ...
    _RYML_CB_ASSERT_BASIC(this->callbacks(), proc.curr() == ' ' || proc.curr() == '\t');
...
{
    _RYML_CB_ASSERT_BASIC(this->callbacks(), proc.curr() == '\n');
...
{
    _RYML_CB_ASSERT_BASIC(this->callbacks(), indentation != npos);
...
void ParseEngine<EventHandler>::_filter_dquoted_backslash(FilterProcessor &C4_RESTRICT proc)
{
    ...
    else if(next == 'u') // UTF16
    {
        ...
        _RYML_CB_ASSERT_BASIC(callbacks(), numbytes <= 4);
...
{
    _RYML_CB_ASSERT_BASIC(this->callbacks(), chomp == CHOMP_CLIP || chomp == CHOMP_KEEP || chomp == CHOMP_STRIP);
    _RYML_CB_ASSERT_BASIC(this->callbacks(), proc.rem().first_not_of(" \n\r") == npos);
    ...
    // advance to the last line having spaces beyond the indentation
    {
        ...
        if(last != npos)
        {
            ...
            _RYML_CB_ASSERT_BASIC(this->callbacks(), last <= proc.src.len);
...
size_t ParseEngine<EventHandler>::_extend_to_chomp(FilterProcessor &C4_RESTRICT proc, size_t contents_len)
{
    ...
    _RYML_CB_ASSERT_BASIC(this->callbacks(), contents_len > 0u);
...
size_t ParseEngine<EventHandler>::_filter_block_folded_newlines_compress(FilterProcessor &C4_RESTRICT proc, size_t num_newl, size_t wpos_at_first_newl)
{
    ...
    case 2u:
        ...
        _RYML_CB_ASSERT_BASIC(this->callbacks(), wpos_at_first_newl != npos);
        _RYML_CB_ASSERT_BASIC(this->callbacks(), proc.sofar()[wpos_at_first_newl] == ' ');
        _RYML_CB_ASSERT_BASIC(this->callbacks(), wpos_at_first_newl + 1u == proc.wpos);
        proc.skip();
...
{
    _RYML_CB_ASSERT_BASIC(this->callbacks(), (proc.rem().first_not_of(" \t") == curr_indentation) || (proc.rem().first_not_of(" \t") == npos));
    if(curr_indentation)
...
csubstr ParseEngine<EventHandler>::_filter_scalar_plain(substr s, size_t indentation)
{
    ...
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, r.valid());
...
csubstr ParseEngine<EventHandler>::_filter_scalar_squot(substr s)
{
    ...
    _RYML_CB_ASSERT_BASIC(this->callbacks(), r.valid());
...
csubstr ParseEngine<EventHandler>::_filter_scalar_dquot(substr s)
{
    ...
    else
    {
        ...
        _RYML_CB_ASSERT_BASIC(this->callbacks(), dst.len == len);
        ...
        _RYML_CB_ASSERT_BASIC(this->callbacks(), rsd.required_len() <= len); // may be smaller!
        _RYML_CB_CHECK_BASIC(m_evt_handler->m_stack.m_callbacks, rsd.valid());
...
{
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, loc.offset < m_buf.len);
...
{
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, node.readable());
...
bool ParseEngine<EventHandler>::_location_from_node(Tree const& tree, id_type node, Location *C4_RESTRICT loc, id_type level) const
{
    if(tree.has_key(node))
    {
        ...
        {
            _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, k.is_sub(m_buf));
            _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, m_buf.is_super(k));
    ...
    if(tree.has_val(node))
    {
        ...
        {
            _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, v.is_sub(m_buf));
            _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, m_buf.is_super(v));
...
{
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, tree.is_container(node));
...
Location ParseEngine<EventHandler>::val_location(const char *val) const
{
    if(C4_UNLIKELY(val == nullptr))
        return {m_file, 0, 0, 0};
    _RYML_CB_CHECK_BASIC(m_evt_handler->m_stack.m_callbacks, m_options.locations());
    // NOTE: if any of these checks fails, the parser needs to be
    // instantiated with locations enabled.
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, m_buf.str == m_newline_offsets_buf.str);
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, m_buf.len == m_newline_offsets_buf.len);
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, m_options.locations());
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, !_locations_dirty());
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, m_newline_offsets != nullptr);
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, m_newline_offsets_size > 0);
    // NOTE: the pointer needs to belong to the buffer that was used to parse.
    csubstr src = m_buf;
    _RYML_CB_CHECK_BASIC(m_evt_handler->m_stack.m_callbacks, val != nullptr || src.str == nullptr);
    _RYML_CB_CHECK_BASIC(m_evt_handler->m_stack.m_callbacks, (val >= src.begin() && val <= src.end()) || (src.str == nullptr && val == nullptr));
    ...
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, lineptr >= m_newline_offsets);
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, lineptr <= m_newline_offsets + m_newline_offsets_size);
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, *lineptr > offset);
    Location loc;
    loc.name = m_file;
    loc.offset = offset;
...
void ParseEngine<EventHandler>::_prepare_locations()
{
    ...
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, m_newline_offsets_size == numnewlines);
}
...
bool ParseEngine<EventHandler>::_handle_indentation_from_annotations()
{
    ...
    {
        _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, m_pending_anchors.num_entries < 2u && m_pending_tags.num_entries < 2u);
        _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, m_pending_anchors.annotations[0].line < m_evt_handler->m_curr->pos.line);
        _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, m_pending_tags.annotations[1].line < m_evt_handler->m_curr->pos.line);
...
void ParseEngine<EventHandler>::_handle_annotations_and_indentation_after_start_mapblck(size_t key_indentation, size_t key_line)
{
    ...
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, m_pending_tags.num_entries <= 2);
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, m_pending_anchors.num_entries <= 2);
    if(m_pending_anchors.num_entries || m_pending_tags.num_entries)
    {
...
{
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, m_pending_tags.num_entries || m_pending_anchors.num_entries);
...
{
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, rem.is_sub(m_evt_handler->m_curr->line_contents.rem));
...
seqjson_start:
    ...
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_none(RKEY));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_all(RSEQ));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_all(FLOW));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_any(RVAL|RNXT));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_all(RVAL) != has_all(RNXT));
    ...
    if(has_any(RVAL))
    {
        _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_none(RNXT));
    ...
    {
        _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_any(RNXT));
        _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_none(RVAL));
        const char first = rem.str[0];
...
mapjson_start:
    ...
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_all(RMAP));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_all(FLOW));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_none(QMRK));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_any(RKEY|RKCL|RVAL|RNXT));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, 1 == (has_any(RKEY) + has_any(RKCL) + has_any(RVAL) + has_any(RNXT)));
    ...
    if(has_any(RKEY))
    {
        _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_none(RKCL));
        _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_none(RVAL));
        _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_none(RNXT));
        _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_none(QMRK));
        const char first = rem.str[0];
...
seqimap_start:
    ...
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_all(RSEQIMAP));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_none(RKEY));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_any(RVAL|RNXT|QMRK|RKCL));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, 1 == has_all(RVAL) + has_all(RNXT) + has_all(QMRK) + has_all(RKCL));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, m_evt_handler->m_stack.size() >= 3);
    ...
    {
        _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_any(QMRK));
        _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_none(RVAL));
        _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_none(RNXT));
        _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_none(RKCL));
        const char first = rem.str[0];
    ...
    else if(has_any(RKCL))
    {
        ...
        _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_none(RNXT));
        _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_none(QMRK));
        _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_any(RKCL));
        const char first = rem.str[0];
...
seqflow_start:
    ...
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_any(RVAL|RNXT));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_all(RVAL) != has_all(RNXT));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, m_evt_handler->m_curr->indref != npos);
...
mapflow_start:
    ...
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_all(RMAP));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_all(FLOW));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_any(RKEY|RKCL|RVAL|RNXT|QMRK));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, 1 == (has_any(RKEY) + has_any(RKCL) + has_any(RVAL) + has_any(RNXT) + has_any(QMRK)));
...
seqblck_start:
    ...
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_all(RSEQ));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_all(BLCK));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_any(RVAL|RNXT));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, 1 == (has_any(RVAL) + has_any(RNXT)));
    ...
    if(has_any(RVAL))
    {
        ...
        #endif
        _RYML_CB_ASSERT_BASIC(callbacks(), rem.len);
        ...
        else if(first == '-')
        {
            ...
            else
            {
                ...
                _RYML_CB_ASSERT_BASIC(this->callbacks(), startindent > m_evt_handler->m_curr->indref);
                addrem_flags(RNXT, RVAL);
                _handle_annotations_before_blck_val_scalar();
    ...
    else // RNXT
    {
        ...
        else
        {
            ...
            if(m_evt_handler->m_parent && has_all(RMAP|BLCK, m_evt_handler->m_parent) && m_evt_handler->m_curr->indref == m_evt_handler->m_parent->indref)
            {
                ...
                _RYML_CB_ASSERT_BASIC(this->callbacks(), m_evt_handler->m_curr != m_evt_handler->m_parent);
                _handle_indentation_pop(m_evt_handler->m_parent);
                _RYML_CB_ASSERT_BASIC(this->callbacks(), has_all(RMAP|BLCK));
                m_evt_handler->add_sibling();
                addrem_flags(RKEY, RNXT);
                goto seqblck_finish;
            }
...
mapblck_start:
        ...
        if(m_evt_handler->m_curr->at_line_beginning())
        {
            ...
            else if(m_evt_handler->m_curr->indentation_lt())
            {
                ...
                if(has_all(RMAP|BLCK))
                {
                    ...
                    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_any(RKEY));
                    rem = m_evt_handler->m_curr->line_contents.rem;
                    if(!rem.len)
                        goto mapblck_again;
                }
            ...
            {
                _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, m_evt_handler->m_curr->indentation_gt());
    ...
    else if(has_any(RKCL)) // read the key colon
    {
        ...
        else if(first == '?')
        {
            ...
            _RYML_CB_CHECK_BASIC(m_evt_handler->m_stack.m_callbacks, m_was_inside_qmrk);
            m_evt_handler->set_val_scalar_plain_empty();
            m_evt_handler->add_sibling();
        ...
        else if(first == '-')
        {
            if(m_evt_handler->m_curr->indref == 0 || m_evt_handler->m_curr->line_contents.indentation == 0 || _is_doc_begin_token(rem))
            {
                ...
                _RYML_CB_CHECK_BASIC(m_evt_handler->m_stack.m_callbacks, _is_doc_begin_token(rem));
                _start_doc_suddenly();
                _line_progressed(3);
                _maybe_skip_whitespace_tokens();
                goto mapblck_finish;
        ...
        {
            _RYML_CB_CHECK_BASIC(m_evt_handler->m_stack.m_callbacks, m_evt_handler->m_curr->indentation_eq());
    ...
    else if(has_any(RVAL))
    {
        ...
        else if(first == '-')
        {
            ...
            else if(m_evt_handler->m_curr->indref == 0 || m_evt_handler->m_curr->line_contents.indentation == 0 || _is_doc_begin_token(rem))
            {
                ...
                _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, _is_doc_begin_token(rem));
                _start_doc_suddenly();
                _line_progressed(3);
                _maybe_skip_whitespace_tokens();
                goto mapblck_finish;
        ...
        else if(first == '*')
        {
            ...
            else
            {
                ...
                _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, startindent > m_evt_handler->m_curr->indref);
                if(_maybe_scan_following_colon())
                {
    ...
    else if(has_any(RNXT))
    {
        ...
        // handle tokens
        //
        _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, rem.len > 0);
        const char first = rem.str[0];
    ...
    else if(has_any(QMRK))
    {
        ...
        if(m_evt_handler->m_curr->at_line_beginning())
        {
            _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, m_evt_handler->m_curr->line_contents.indentation != npos);
            if(m_evt_handler->m_curr->indentation_eq())
            {
...
void ParseEngine<EventHandler>::_handle_unk_json()
{
    ...
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_none(RNXT|RSEQ|RMAP));
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_all(RTOP));
    ...
    {
        _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks,  ! has_any(SSCL));
        _maybe_skip_whitespace_tokens();
        csubstr s = m_evt_handler->m_curr->line_contents.rem;
        if(!s.len)
            return;
...
C4_COLD void ParseEngine<EventHandler>::_handle_usty()
{
    ...
    _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks, has_none(BLCK|FLOW));
...
void ParseEngine<EventHandler>::parse_json_in_place_ev(csubstr filename, substr src)
{
    ...
    while( ! _finished_file())
    {
        ...
        {
            _c4dbgnextline();
            _RYML_CB_ASSERT_BASIC(m_evt_handler->m_stack.m_callbacks,  ! m_evt_handler->m_curr->line_contents.rem.empty());
            if(has_any(RSEQ))
            {
                _handle_seq_json();
            }
