...
inline bool _is_blck_token(csubstr s) noexcept
{
    RYML_ASSERT(s.len > 0);
...
void ParseEngine<EventHandler>::_fmt_msg(DumpFn &&dumpfn) const
{
    ...
    if(contents.len)
    {
        ...
        if(m_file.len)
        {
            detail::_parse_dump(dumpfn, "{}:", m_file);
            offs += m_file.len + 1;
        }
        detail::_parse_dump(dumpfn, "{}:{}: ", st->pos.line, st->pos.col);
...
void ParseEngine<EventHandler>::_err(csubstr fmt, Args const& C4_RESTRICT ...args) const
{
    ...
    m_evt_handler->m_stack.m_callbacks.m_error(errmsg, len, m_state->pos, m_evt_handler->m_stack.m_callbacks.m_user_data);
}
...
//-----------------------------------------------------------------------------
template<class EventHandler>
void ParseEngine<EventHandler>::_handle_finished_file()
{
    _end_stream();
}
...
template<class EventHandler>
C4_ALWAYS_INLINE void ParseEngine<EventHandler>::_skipchars(char c)
{
    _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, m_state->line_contents.rem.begins_with(c));
...
template<class EventHandler>
C4_ALWAYS_INLINE void ParseEngine<EventHandler>::_maybe_skipchars_up_to(char c, size_t max_to_skip)
{
    csubstr rem = m_state->line_contents.rem;
    if(rem.len && rem.str[0] == c)
    {
...
template<class EventHandler>
template<size_t N>
C4_ALWAYS_INLINE void ParseEngine<EventHandler>::_skipchars(const char (&chars)[N])
{
    _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, m_state->line_contents.rem.begins_with_any(chars));
...
template<class EventHandler>
C4_ALWAYS_INLINE bool ParseEngine<EventHandler>::_maybe_scan_following_colon() noexcept
{
...
template<class EventHandler>
csubstr ParseEngine<EventHandler>::_scan2_tag()
{
...
template<class EventHandler>
bool ParseEngine<EventHandler>::_scan2_scalar_plain_seq_flow(ScannedScalar *C4_RESTRICT sc)
{
    ...
    substr s = m_state->line_contents.rem;
...
template<class EventHandler>
bool ParseEngine<EventHandler>::_scan2_scalar_plain_map_flow(ScannedScalar *C4_RESTRICT sc)
{
...
template<class EventHandler>
bool ParseEngine<EventHandler>::_scan2_scalar_seq_json(ScannedScalar *C4_RESTRICT sc)
{
    _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, has_none(RMAP));
    _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, has_none(BLCK));
...
template<class EventHandler>
bool ParseEngine<EventHandler>::_scan2_scalar_map_json(ScannedScalar *C4_RESTRICT sc)
{
...
bool ParseEngine<EventHandler>::_is_doc_begin(csubstr s)
{
    ...
    return (m_state->line_contents.indentation == 0u && _at_line_begin() && _is_doc_begin_token(s));
}
...
{
    _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, s[0] == '.');
    return (m_state->line_contents.indentation == 0u && _at_line_begin() && _is_doc_end_token(s));
}

template<class EventHandler>
bool ParseEngine<EventHandler>::_scan2_scalar_plain_blck(ScannedScalar *C4_RESTRICT sc, size_t indentation)
{
    ...
    while(true)
    {
        ...
        for(size_t i = 0; i < s.len; ++i)
        {
            ...
            case ':':
                ...
                if((i + 1 == s.len) || ((s.str[i+1] == ' ') _RYML_WITH_TAB_TOKENS( || (s.str[i+1] == '\t'))))
                {
                    ...
                    if(C4_LIKELY(m_state->pos.line == start_line))
                    {
        ...
        _line_progressed(s.len);
        csubstr next_peeked = _peek_next_line(m_state->pos.offset);
...
template<class EventHandler>
bool ParseEngine<EventHandler>::_scan2_scalar_plain_seq_blck(ScannedScalar *C4_RESTRICT sc)
{
    _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, has_none(RMAP));
    _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, has_none(FLOW));
    _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, has_none(RSEQIMAP));
    ...
    _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, has_any(RVAL));
    return _scan2_scalar_plain_blck(sc, m_state->indref + 1u);
}

template<class EventHandler>
bool ParseEngine<EventHandler>::_scan2_scalar_plain_map_blck(ScannedScalar *C4_RESTRICT sc)
{
    _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, has_none(RSEQ));
    _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, has_none(FLOW));
    _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, has_any(RMAP));
...
template<class EventHandler>
bool ParseEngine<EventHandler>::_scan2_scalar_plain_unk(ScannedScalar *C4_RESTRICT sc)
{
    _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks,  has_any(RUNK|USTY));
    return _scan2_scalar_plain_blck(sc, m_state->indref);
}
...
{
    if(C4_LIKELY(m_state->pos.offset < m_buf.len))
...
void ParseEngine<EventHandler>::_line_progressed(size_t ahead)
{
    ...
    _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, m_state->pos.col <= m_state->line_contents.stripped.len+1);
...
void ParseEngine<EventHandler>::_line_ended()
{
    _c4dbgpf("line[{}] ({} cols) ended! offset {}-->{} / col {}-->{}",
             ...
             m_state->pos.offset, m_state->pos.offset + m_state->line_contents.full.len - m_state->line_contents.stripped.len,
             m_state->pos.col, 1);
    _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, m_state->pos.col == m_state->line_contents.stripped.len + 1);
...
{
    _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, m_state->pos.col == 1u);
    _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, m_state->pos.line > 0u);
    _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, m_state->pos.offset >= m_state->line_contents.full.len - m_state->line_contents.stripped.len);
    const size_t delta = m_state->line_contents.full.len - m_state->line_contents.stripped.len;
    ...
    //_RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, m_state->pos.offset >= m_buf.len || m_buf[m_state->pos.offset] == '\n' || m_buf[m_state->pos.offset] == '\r');
...
template<class EventHandler>
void ParseEngine<EventHandler>::_save_indentation(size_t behind)
{
    ...
    _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, behind <= m_state->indref);
...
{
    _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, m_evt_handler->m_stack.size() >= 1u);
    if(m_evt_handler->m_stack[0].flags & RDOC)
    {
        _c4dbgp("root is RDOC");
        if(m_state->level != 0)
            _handle2_indentation_pop(&m_evt_handler->m_stack[0]);
    }
    else if((m_evt_handler->m_stack.size() > 1) && (m_evt_handler->m_stack[1].flags & RDOC))
    {
        _c4dbgp("root is STREAM");
        if(m_state->level != 1)
            _handle2_indentation_pop(&m_evt_handler->m_stack[1]);
    }
    else
    {
...
{
    _c4dbgpf("end_stream, level={} node_id={}", m_state->level, m_state->tr_id);
    if(has_all(RSEQ|FLOW))
        _c4err("missing terminating ]");
    else if(has_all(RMAP|FLOW))
        _c4err("missing terminating }");
    if(m_evt_handler->m_stack.size() > 1)
        _handle2_indentation_pop(m_evt_handler->m_stack.begin());
    if(has_all(RDOC))
    {
        _end2_doc();
    }
...
template<class EventHandler>
void ParseEngine<EventHandler>::_handle2_indentation_pop(ParserState const* popto)
{
    _c4dbgpf("popping {} level{}: from level {}(@ind={}) to level {}(@ind={})", m_state->level - popto->level, (((m_state->level - popto->level) > 1) ? "s" : ""), m_state->level, m_state->indref, popto->level, popto->indref);
    while(m_state != popto)
    {
        if(has_any(RSEQ))
        {
            _c4dbgpf("popping seq at level {} (indentation={},addr={})", m_state->level, m_state->indref, m_state);
            _end2_seq();
        }
        else if(has_any(RMAP))
        {
            _c4dbgpf("popping map at level {} (indentation={},addr={})", m_state->level, m_state->indref, m_state);
            _end2_map();
        }
        else
        {
    ...
    _c4dbgpf("current level is {} (indentation={})", m_state->level, m_state->indref);
}

template<class EventHandler>
void ParseEngine<EventHandler>::_handle2_indentation_pop_from_block_seq()
{
    ...
    {
        _c4dbgpf("searching for state with indentation {}. curr={} (level={},node={})", ind, s->indref, s->level, s->tr_id);
        if(s->indref == ind)
        {
            _c4dbgpf("gotit!!! level={} node={}", s->level, s->tr_id);
            popto = s;
            break;
        }
    }
    ...
    _handle2_indentation_pop(popto);
}

template<class EventHandler>
void ParseEngine<EventHandler>::_handle2_indentation_pop_from_block_map()
{
    // search the stack frame to jump to based on its indentation
    using state_type = State;
    auto &stack = m_evt_handler->m_stack;
...
//-----------------------------------------------------------------------------
template<class EventHandler>
typename ParseEngine<EventHandler>::ScannedScalar ParseEngine<EventHandler>::_scan2_scalar_squot()
{
    ...
    while( ! _finished_file())
    {
        ...
        else
        {
            ...
            _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, m_buf[m_state->pos.offset + pos] == '\'');
...
//-----------------------------------------------------------------------------
template<class EventHandler>
typename ParseEngine<EventHandler>::ScannedScalar ParseEngine<EventHandler>::_scan2_scalar_dquot()
{
    // quoted scalars can spread over multiple lines!
    // nice explanation here: http://yaml-multiline.info/
...
//-----------------------------------------------------------------------------
template<class EventHandler>
void ParseEngine<EventHandler>::_scan2_block(ScannedBlock *C4_RESTRICT sb, size_t indref)
{
    ...
    if(s.len > 1)
    {
        ...
        if( ! digits.empty())
        {
            ...
            _c4dbgpf("blck: indentation specified: {}. add {} from curr state -> {}", indentation, m_state->indref, indentation+indref);
    ...
    _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, raw_block.begin() == m_state->line_contents.full.begin());
    ...
    while(( ! _finished_file()))
    {
            ...
            if(lc.stripped.first_not_of(' ') != npos) // non-empty line
            {
                ...
                if(provisional_indentation == npos)
                {
                    ...
                    else if(lc.indentation == m_state->indref)
                    {
                        ...
                        {
                            _c4dbgpf("blck: block terminated. reading container and indentation={}==indref={}", lc.indentation, m_state->indref);
                            break;
                        }
                    }
        ...
        _c4dbgpf("blck: append '{}'", m_state->line_contents.rem);
        raw_block.len += m_state->line_contents.full.len;
        _line_progressed(m_state->line_contents.rem.len);
        _line_ended();
        ++num_lines;
    }
    _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, m_state->pos.line == (first + num_lines) || (raw_block.len == 0));
    C4_UNUSED(num_lines);
    C4_UNUSED(first);
...
template<class EventHandler>
size_t ParseEngine<EventHandler>::_count_nlines(csubstr src)
{
    return 1 + src.count('\n');
}
...
void ParseEngine<EventHandler>::_handle_directive(csubstr directive)
{
    ...
    m_evt_handler->add_directive(directive);
}
...
template<class EventHandler>
void ParseEngine<EventHandler>::set_flags(ParserFlag_t f, ParserState * s)
{
    char buf1_[64], buf2_[64];
    csubstr buf1 = detail::_parser_flags_to_str(buf1_, f);
    csubstr buf2 = detail::_parser_flags_to_str(buf2_, s->flags);
    _c4dbgpf("state[{}]: set to {}: before={}", s->level, buf1, buf2);
    s->flags = f;
}
...
{
    _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, node.valid());
    return location(*node.tree(), node.id());
}

template<class EventHandler>
Location ParseEngine<EventHandler>::location(Tree const& tree, size_t node) const
{
...
template<class EventHandler>
bool ParseEngine<EventHandler>::_location_from_node(Tree const& tree, size_t node, Location *C4_RESTRICT loc, size_t level) const
{
...
template<class EventHandler>
bool ParseEngine<EventHandler>::_location_from_cont(Tree const& tree, size_t node, Location *C4_RESTRICT loc) const
{
...
template<class EventHandler>
void ParseEngine<EventHandler>::_handle2_flow_skip_whitespace()
{
    if(m_state->line_contents.rem.len > 0)
    {
        csubstr rem = m_state->line_contents.rem;
        ...
        if(rem.begins_with('#'))
        {
            _c4dbgpf("it's a comment: {}", m_state->line_contents.rem);
            _line_progressed(m_state->line_contents.rem.len);
        }
    }
}
...
template<class EventHandler>
void ParseEngine<EventHandler>::_add_annotation(ValAnnotation *C4_RESTRICT dst, csubstr str, size_t indentation, size_t line)
{
...
template<class EventHandler>
void ParseEngine<EventHandler>::_clear_annotations(ValAnnotation *C4_RESTRICT dst)
{
    dst->num_entries = 0;
}
...
bool ParseEngine<EventHandler>::_handle_indentation_from_annotations()
{
    if(m_pending_anchors.num_entries == 1u || m_pending_tags.num_entries == 1u)
    {
        ...
        _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, m_pending_anchors.annotations[0].line < m_state->pos.line);
        _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, m_pending_tags.annotations[1].line < m_state->pos.line);
        size_t to_skip = m_state->indref;
...
template<class EventHandler>
bool ParseEngine<EventHandler>::_handle2_seq_json()
{
seqjson_start:
    _c4dbgpf("handle2_seq_json: node_id={} level={} indentation={}", m_state->tr_id, m_state->level, m_state->indref);
    ...
    if(has_any(RVAL))
    {
        ...
        else if(_scan2_scalar_seq_json(&sc))
        {
            _c4dbgp("seqjson[RVAL]: it's a scalar.");
            csubstr maybe_filtered = _maybe_filter_val_scalar_plain(sc, m_state->indref);
...
template<class EventHandler>
bool ParseEngine<EventHandler>::_handle2_map_json()
{
mapjson_start:
    _c4dbgpf("handle2_map_json: node_id={} level={} indentation={}", m_state->tr_id, m_state->level, m_state->indref);
    ...
    else if(has_any(RVAL))
    {
        ...
        else if(_scan2_scalar_map_json(&sc))
        {
    ...
    else if(has_any(RKCL)) // read the key colon
    {
        ...
        {
            _c4dbgp("mapjson[RKCL]: end with missing val!");
            addrem_flags(RVAL, RKCL);
            m_evt_handler->set_val_scalar_plain({});
            m_evt_handler->end_map();
            _line_progressed(1);
        ...
        {
            _c4dbgp("mapjson[RKCL]: got comma. val is missing");
            m_evt_handler->set_val_scalar_plain({});
            m_evt_handler->add_sibling();
...
template<class EventHandler>
bool ParseEngine<EventHandler>::_handle2_seq_imap()
{
seqimap_start:
    _c4dbgpf("handle2_seq_imap: node_id={} level={} indref={}", m_state->tr_id, m_state->level, m_state->indref);
    ...
    if(has_any(RVAL))
    {
        ...
        else if(_scan2_scalar_plain_map_flow(&sc))
        {
    ...
    else if(has_any(QMRK))
    {
        ...
        else if(_scan2_scalar_plain_map_flow(&sc))
        {
            ...
            csubstr maybe_filtered = _maybe_filter_key_scalar_plain(sc, m_state->indref);
...
template<class EventHandler>
bool ParseEngine<EventHandler>::_handle2_seq_flow()
{
seqflow_start:
    _c4dbgpf("handle2_seq_flow: node_id={} level={} indentation={}", m_state->tr_id, m_state->level, m_state->indref);
    ...
    _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, has_any(RVAL|RNXT));
    _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, has_all(RVAL) != has_all(RNXT));
    _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, m_state->indref != npos);
    ...
    if(has_any(RVAL))
    {
        ...
        // block scalars (ie | and >) cannot appear in flow containers
        else if(_scan2_scalar_plain_seq_flow(&sc))
        {
        ...
        {
            csubstr tag = _scan2_tag();
            _c4dbgpf("seqflow[RVAL]: tag! [{}]~~~{}~~~", tag.len, tag);
            m_evt_handler->set_val_tag(tag);
        }
        else if(first == ':')
        {
            _c4dbgpf("seqflow[RVAL]: actually seqimap at node[{}], with empty key", m_state->tr_id);
            addrem_flags(RNXT, RVAL);
            m_evt_handler->begin_map_val_flow();
            _set_indentation(m_evt_handler->m_parent->indref);
            m_evt_handler->set_key_scalar_plain({});
    ...
    else // RNXT
    {
        ...
        {
            _c4dbgpf("seqflow[RNXT]: actually seqimap at node[{}]", m_state->tr_id);
...
template<class EventHandler>
bool ParseEngine<EventHandler>::_handle2_map_flow()
{
mapflow_start:
    _c4dbgpf("handle2_map_flow: node_id={} level={} indentation={}", m_state->tr_id, m_state->level, m_state->indref);
    ...
    if(has_any(RKEY))
    {
        ...
        else if(first == '}') // this happens on a trailing comma like ", }"
        {
            ...
            if(has_any(QMRK))
            {
                _c4dbgp("mapflow[RKEY|QMRK]: add missing keyval!");
                m_evt_handler->set_key_scalar_plain({});
                m_evt_handler->set_val_scalar_plain({});
            }
            m_evt_handler->end_map();
...
template<class EventHandler>
bool ParseEngine<EventHandler>::_handle2_seq_block()
{
seqblck_start:
    _c4dbgpf("handle2_seq_block: seq_id={} node_id={} level={} indent={}", m_evt_handler->m_parent->tr_id, m_state->tr_id, m_state->level, m_state->indref);
    ...
    if(has_any(RVAL))
    {
        _c4dbgpf("seqblck[RVAL]: col={}", m_state->pos.col);
        _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, has_none(RNXT));
        if(m_state->at_line_beginning())
        {
            _c4dbgpf("seqblck[RVAL]: indref={} indentation={}", m_state->indref, m_state->line_contents.indentation);
            if(m_state->indentation_ge())
            {
                _c4dbgpf("seqblck[RVAL]: skip {} from indentation", m_state->line_contents.indentation);
                _line_progressed(m_state->line_contents.indentation);
                rem = m_state->line_contents.rem;
                if(!rem.len)
                    goto seqblck_again;
            }
            else if(m_state->indentation_lt())
            {
                _c4dbgp("seqblck[RVAL]: smaller indentation!");
                _handle2_indentation_pop_from_block_seq();
                goto seqblck_finish;
            }
            else if(m_state->line_contents.indentation == npos)
            {
        ...
        const char first = rem.str[0];
        const size_t startline = m_state->pos.line;
        const size_t startindent = m_state->line_contents.current_col();
        _c4dbgpf("seqblck[RVAL]: '{}' node_id={}", first, m_state->tr_id);
        ScannedScalar sc;
        if(first == '\'')
        {
        ...
        else if(first == '|')
        {
            ...
            ScannedBlock sb;
            _scan2_block(&sb, m_state->indref + 1);
            _handle_annotations_before_blck_val_scalar();
        ...
        else if(_scan2_scalar_plain_seq_blck(&sc))
        {
            ...
            if(!_maybe_scan_following_colon())
            {
                _c4dbgp("seqblck[RVAL]: set as val");
                _handle_annotations_before_blck_val_scalar();
                csubstr maybe_filtered = _maybe_filter_val_scalar_plain(sc, m_state->indref);  // VAL!
                m_evt_handler->set_val_scalar_plain(maybe_filtered);
                addrem_flags(RNXT, RVAL);
            }
            else
            {
                if(startindent > m_state->indref)
                {
                    _c4dbgp("seqblck[RVAL]: start mapblck, set scalar as key");
                    addrem_flags(RNXT, RVAL);
                    _handle_annotations_before_start_mapblck(startline);
        ...
        else if(first == '-')
        {
            ...
            else
            {
                ...
                _RYML_CB_ASSERT(this->callbacks(), startindent > m_state->indref);
                addrem_flags(RNXT, RVAL);
                _handle_annotations_before_blck_val_scalar();
        ...
        {
            const csubstr anchor = _scan2_anchor();
    ...
    else // RNXT
    {
        ...
        {
            _c4dbgpf("seqblck[RNXT]: blank line, len={}", m_state->line_contents.rem);
            _line_progressed(m_state->line_contents.rem.len);
            rem = m_state->line_contents.rem;
            if(!rem.len)
                goto seqblck_again;
        ...
        //
        const char first = rem.str[0];
        _c4dbgpf("seqblck[RNXT]: '{}' node_id={}", first, m_state->tr_id);
        if(first == '-')
        {
        ...
        else
        {
            ...
            #endif
            if(m_evt_handler->m_parent && has_all(RMAP|BLCK, m_evt_handler->m_parent) && m_state->indref == m_evt_handler->m_parent->indref)
            {
                _c4dbgpf("seqblck[RNXT]: end indentless seq, go to parent={}. node={}", m_evt_handler->m_parent->tr_id, m_state->tr_id);
                _RYML_CB_ASSERT(this->callbacks(), m_state != m_evt_handler->m_parent);
                _handle2_indentation_pop(m_evt_handler->m_parent);
...
template<class EventHandler>
bool ParseEngine<EventHandler>::_handle2_map_block()
{
mapblck_start:
    _c4dbgpf("handle2_map_block: map_id={} node_id={} level={} indref={}", m_evt_handler->m_parent->tr_id, m_state->tr_id, m_state->level, m_state->indref);
    ...
    if(has_any(RKEY))
    {
        ...
        if(m_state->at_line_beginning())
        {
            ...
            {
                _c4dbgpf("mapblck[RKEY]: skip {} from indref", m_state->indref);
                _line_progressed(m_state->indref);
                rem = m_state->line_contents.rem;
                if(!rem.len)
                    goto mapblck_again;
            ...
            else if(m_state->indentation_lt())
            {
                ...
                _handle2_indentation_pop_from_block_map();
                _line_progressed(m_state->line_contents.indentation);
            ...
            {
                _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, m_state->indentation_gt());
        ...
        // so no need to check for them here.
        else if(_scan2_scalar_plain_map_blck(&sc))
        {
        ...
        else if(first == '{')
        {
            ...
            // handled inside the tree sink. Other sink types may be
            // able to handle it.
            _c4dbgp("mapblck[RKEY]: start child mapblck (!)");
            m_evt_handler->begin_map_key_flow();
            addrem_flags(RKEY, RVAL|RKCL);
            _line_progressed(1);
            // keep going in this function
    ...
    else if(has_any(RKCL)) // read the key colon
    {
        ...
        if(m_state->at_line_beginning())
        {
            if(m_state->indentation_eq())
            {
                _c4dbgpf("mapblck[RKCL]: skip {} from indref", m_state->indref);
                _line_progressed(m_state->indref);
                rem = m_state->line_contents.rem;
                if(!rem.len)
                    goto mapblck_again;
            }
            else if(C4_UNLIKELY(m_state->indentation_lt()))
            {
                _c4err("invalid indentation");
            }
        }
        ...
        {
            _RYML_CB_CHECK(m_evt_handler->m_stack.m_callbacks, m_state->indentation_eq());
    ...
    else if(has_any(RVAL))
    {
        ...
        if(m_state->at_line_beginning())
        {
            _c4dbgpf("mapblck[RVAL]: indref={} indentation={}", m_state->indref, m_state->line_contents.indentation);
            m_state->more_indented = false;
            if(m_state->indref == npos)
            {
            ...
            else if(m_state->indentation_eq())
            {
            ...
            else if(m_state->indentation_gt())
            {
                _c4dbgp("mapblck[RVAL]: more indented!");
                m_state->more_indented = true;
                _line_progressed(m_state->line_contents.indentation);
                rem = m_state->line_contents.rem;
                if(!rem.len)
                    goto mapblck_again;
        ...
        else if(_scan2_scalar_plain_map_blck(&sc))
        {
            ...
            else
            {
                if(startindent != m_state->indref)
                {
                    _c4dbgpf("mapblck[RVAL]: start new block map, set scalar as key {}", m_state->indref);
                    addrem_flags(RNXT, RVAL);
                    _handle_annotations_before_start_mapblck(startline);
                    m_evt_handler->begin_map_val_block();
                    _handle_annotations_and_indentation_after_start_mapblck(startindent, startline);
        ...
        else if(first == '[')
        {
            ...
            _set_indentation(m_state->indref + 1u);
            _line_progressed(1);
            goto mapblck_finish;
        }
        else if(first == '{')
        {
            ...
            m_state->scalar_col = m_state->line_contents.indentation;
            _set_indentation(m_state->indref + 1u);
            _line_progressed(1);
            goto mapblck_finish;
        }
        else if(first == '*')
        {
            ...
            {
                _c4dbgpf("mapblck[RVAL]: larger indentation {}>{}", startindent, m_state->indref);
                _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, startindent > m_state->indref);
                if(_maybe_scan_following_colon())
                {
        ...
        else if(first == '?')
        {
            ...
            else if(startindent > m_state->indref)
            {
    ...
    else if(has_any(RNXT))
    {
        ...
        if(m_state->at_line_beginning())
        {
            _c4dbgpf("mapblck[RNXT]: indref={} indentation={}", m_state->indref, m_state->line_contents.indentation);
            if(m_state->indentation_eq())
            {
                _c4dbgpf("mapblck[RNXT]: skip {} from indref", m_state->indref);
                _line_progressed(m_state->indref);
...
template<class EventHandler>
bool ParseEngine<EventHandler>::_handle_unk_json()
{
    _c4dbgpf("handle_unk_json indref={} target={}", m_state->indref, m_state->tr_id);
    ...
    else
    {
        ...
        if(!s.len)
            return true;
        const size_t startindent = m_state->line_contents.indentation; // save
        const size_t startline = m_state->pos.line; // save
        ...
        if(first == '"')
        {
            ...
            {
                _c4dbgp("runk_json: start new block map, set double-quoted scalar as key");
                _handle_annotations_before_start_mapblck(startline);
                m_evt_handler->begin_map_val_block();
                _handle_annotations_and_indentation_after_start_mapblck(startindent, startline);
                m_evt_handler->set_key_scalar_dquoted(maybe_filtered);
        ...
        else if(_scan2_scalar_plain_unk(&sc))
        {
            ...
            {
                _c4dbgp("runk_json: start new block map, set scalar as key");
                _handle_annotations_before_start_mapblck(startline);
                m_evt_handler->begin_map_val_block();
                _handle_annotations_and_indentation_after_start_mapblck(startindent, startline);
...
template<class EventHandler>
bool ParseEngine<EventHandler>::_handle_unk()
{
    _c4dbgpf("handle_unk indref={} target={}", m_state->indref, m_state->tr_id);
    ...
    if(m_state->line_contents.indentation == 0u && _at_line_begin())
    {
        const char first = rem.str[0];
    ...
    if(rem.begins_with('['))
    {
        ...
        _maybe_begin_doc();
        m_2doc_empty = false;
        const size_t startindent = m_state->line_contents.current_col(rem);
        ...
        else
        {
            ...
            _handle_annotations_and_indentation_after_start_mapblck(startindent, m_state->pos.line);
            m_evt_handler->begin_seq_key_flow();
    ...
    else if(rem.begins_with('{'))
    {
        ...
        else
        {
            ...
            _handle_annotations_and_indentation_after_start_mapblck(startindent, m_state->pos.line);
            m_evt_handler->begin_map_key_flow();
            addrem_flags(RMAP|FLOW|RKEY, RMAP|BLCK|RKCL);
            _set_indentation(startindent);
        }
        _line_progressed(1);
        return true;
    ...
    else if(rem == ':' || rem.begins_with(": ") _RYML_WITH_TAB_TOKENS( || rem.begins_with(":\t")))
    {
        ...
        else
        {
            ...
            size_t prev_indentation = m_state->indref;
    ...
    else if(rem.begins_with('&'))
    {
        ...
        m_evt_handler->check_trailing_doc_token();
        _maybe_begin_doc();
        const size_t indentation = m_state->line_contents.current_col(rem);
        const size_t line = m_state->pos.line;
    ...
    else
    {
        ...
        else if(first == '|')
        {
            ...
            {
                _c4dbgp("runk: start new block map, set block-literal scalar as key");
                _handle_annotations_before_start_mapblck(startline);
                m_evt_handler->begin_map_val_block();
                _handle_annotations_and_indentation_after_start_mapblck(startindent, startline);
        ...
        else if(first == '>')
        {
            ...
            {
                _c4dbgp("runk: start new block map, set block-folded scalar as key");
                _handle_annotations_before_start_mapblck(startline);
                m_evt_handler->begin_map_val_block();
                _handle_annotations_and_indentation_after_start_mapblck(startindent, startline);
...
template<class EventHandler>
bool ParseEngine<EventHandler>::_handle_usty()
{
    _c4dbgpf("handle_usty target={}", m_state->indref, m_state->tr_id);
    ...
    _RYML_CB_ASSERT(m_evt_handler->m_stack.m_callbacks, rem.len > 0);
    size_t startindent = m_state->line_contents.indentation; // save
    ...
    if(has_any(RSEQ))
    {
        ...
        {
            _c4err("cannot parse a map into an existing seq");
        }
        else
        {
            _c4err("parse error. can only parse a seq into an existing seq");
        }
    }
    else if(has_any(RMAP))
    {
        ...
        {
            // maybe this is a (squo/dquo/lit/fold/plain)-scalar, or
            // an anchor, or a ref, or a tag. Whatever it is, we just
            // assume this is a block map, and hope for the best while
            // parsing.
            _c4err("usty[RMAP]: assuming block map");
            add_flags(RNXT);
            m_evt_handler->_push();
            addrem_flags(RMAP|BLCK|RKEY, RNXT|USTY);
            _set_indentation(startindent);
            // don't progress the line
        }
    }
    else
    {
        ...
        else
        {
            ...
            if(!s.len)
                return true;
            startindent = m_state->line_contents.indentation; // save
            const size_t startline = m_state->pos.line; // save
            first = s.str[0];
            ScannedScalar sc;
            ...
            else if(first == '|')
            {
                ...
                {
                    _c4dbgp("usty[UNK]: start new block map, set block-literal scalar as key");
                    add_flags(RNXT);
                    _handle_annotations_before_start_mapblck(startline);
                    m_evt_handler->begin_map_val_block();
                    _handle_annotations_and_indentation_after_start_mapblck(startindent, startline);
            ...
            else if(first == '>')
            {
                ...
                {
                    _c4dbgp("usty[UNK]: start new block map, set block-folded scalar as key");
                    add_flags(RNXT);
                    _handle_annotations_before_start_mapblck(startline);
                    m_evt_handler->begin_map_val_block();
                    _handle_annotations_and_indentation_after_start_mapblck(startindent, startline);
