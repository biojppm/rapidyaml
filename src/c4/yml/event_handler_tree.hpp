...
public:
    ...
    {
        if(C4_UNLIKELY(!tree))
            _RYML_CB_ERR_BASIC(m_stack.m_callbacks, "null tree");
        if(C4_UNLIKELY(id >= tree->capacity()))
            _RYML_CB_ERR_BASIC(tree->callbacks(), "invalid node");
        if(C4_UNLIKELY(!tree->is_root(id)))
            if(C4_UNLIKELY(tree->is_map(tree->parent(id))))
                if(C4_UNLIKELY(!tree->has_key(id)))
                    _RYML_CB_ERR_BASIC(tree->callbacks(), "destination node belongs to a map and has no key");
        m_tree = tree;
        m_id = id;
...
public:
    ...
    {
        _RYML_CB_ASSERT_BASIC(m_stack.m_callbacks, m_tree != nullptr);
    ...
    void finish_parse()
    {
        ...
        if(m_num_directives && !m_tree->is_stream(m_tree->root_id()))
            _RYML_CB_ERR_BASIC(m_stack.m_callbacks, "directives cannot be used without a document");
...
public:
    ...
    void begin_doc_expl()
    {
        ...
        _RYML_CB_ASSERT_VISIT(m_stack.m_callbacks, m_tree->root_id() == m_curr->node_id, m_tree, m_curr->node_id);
        if(!m_tree->is_stream(m_tree->root_id())) //if(_should_push_on_begin_doc())
        {
            ...
            _RYML_CB_ASSERT_VISIT(m_stack.m_callbacks, m_tree->is_stream(m_tree->root_id()), m_tree, m_curr->node_id);
            _RYML_CB_ASSERT_VISIT(m_stack.m_callbacks, m_tree->num_children(m_tree->root_id()) == 1u, m_tree, m_curr->node_id);
...
public:
    ...
    {
        _RYML_CB_ERR_PARSE(m_stack.m_callbacks, m_curr->pos, "ryml trees cannot handle containers as keys");
    }
...
public:
    ...
    void add_sibling()
    {
        _RYML_CB_ASSERT_BASIC(m_stack.m_callbacks, m_tree);
        _RYML_CB_ASSERT_BASIC(m_stack.m_callbacks, m_parent);
        _RYML_CB_ASSERT_VISIT(m_stack.m_callbacks, m_tree->has_children(m_parent->node_id), m_tree, m_parent->node_id);
    ...
    void actually_val_is_first_key_of_new_map_flow()
    {
        if(C4_UNLIKELY(m_tree->is_container(m_curr->node_id)))
            _RYML_CB_ERR_PARSE(m_stack.m_callbacks, m_curr->pos, "ryml trees cannot handle containers as keys");
        _RYML_CB_ASSERT_BASIC(m_stack.m_callbacks, m_parent);
        _RYML_CB_ASSERT_VISIT(m_stack.m_callbacks, m_tree->is_seq(m_parent->node_id), m_tree, m_parent->node_id);
        _RYML_CB_ASSERT_VISIT(m_stack.m_callbacks, !m_tree->is_container(m_curr->node_id), m_tree, m_curr->node_id);
        _RYML_CB_ASSERT_VISIT(m_stack.m_callbacks, !m_tree->has_key(m_curr->node_id), m_tree, m_curr->node_id);
...
public:
    ...
    void set_key_anchor(csubstr anchor)
    {
        ...
        if(C4_UNLIKELY(_has_any_(KEYREF)))
            _RYML_CB_ERR_PARSE(m_tree->callbacks(), m_curr->pos, "key cannot have both anchor and ref");
        _RYML_CB_ASSERT_PARSE(m_tree->callbacks(), !anchor.begins_with('&'), m_curr->pos);
        _enable_(KEYANCH);
    ...
    void set_val_anchor(csubstr anchor)
    {
        ...
        if(C4_UNLIKELY(_has_any_(VALREF)))
            _RYML_CB_ERR_PARSE(m_tree->callbacks(), m_curr->pos, "val cannot have both anchor and ref");
        _RYML_CB_ASSERT_PARSE(m_tree->callbacks(), !anchor.begins_with('&'), m_curr->pos);
        _enable_(VALANCH);
    ...
    void set_key_ref(csubstr ref)
    {
        ...
        if(C4_UNLIKELY(_has_any_(KEYANCH)))
            _RYML_CB_ERR_PARSE(m_tree->callbacks(), m_curr->pos, "key cannot have both anchor and ref");
        _RYML_CB_ASSERT_PARSE(m_tree->callbacks(), ref.begins_with('*'), m_curr->pos);
        _enable_(KEY|KEYREF);
...
public:
    ...
    C4_NO_INLINE void add_directive(csubstr directive)
    {
        ...
        _RYML_CB_ASSERT_PARSE(m_tree->callbacks(), directive.begins_with('%'), m_curr->pos);
...
public:
    ...
    void _reset_parser_state(state* st, id_type parse_root, id_type node)
    {
        ...
        {
            _RYML_CB_ERR_VISIT(m_tree->callbacks(), m_tree, node, "cannot append to node");
        }
        if(type.is_doc())
        {
...
public:
    ...
    void _set_root_as_stream()
    {
        ...
        _RYML_CB_ASSERT_VISIT(m_tree->callbacks(), m_tree->root_id() == 0u, m_tree, m_tree->root_id());
        _RYML_CB_ASSERT_VISIT(m_tree->callbacks(), m_curr->node_id == 0u, m_tree, m_curr->node_id);
        ...
        _RYML_CB_ASSERT_VISIT(m_tree->callbacks(), m_tree->is_stream(m_tree->root_id()), m_tree, m_tree->root_id());
        _RYML_CB_ASSERT_VISIT(m_tree->callbacks(), m_tree->has_children(m_tree->root_id()), m_tree, m_tree->root_id());
        _RYML_CB_ASSERT_VISIT(m_tree->callbacks(), m_tree->is_doc(m_tree->first_child(m_tree->root_id())), m_tree, m_tree->root_id());
        if(hack)
    ...
    void _remove_speculative()
    {
        ...
        _RYML_CB_ASSERT_BASIC(m_stack.m_callbacks, m_tree);
        _RYML_CB_ASSERT_BASIC(m_tree->callbacks(), !m_tree->empty());
    ...
    void _remove_speculative_with_parent()
    {
        ...
        _RYML_CB_ASSERT_BASIC(m_tree->callbacks(), !m_tree->empty());
        const id_type last_added = m_tree->size() - 1;
        _RYML_CB_ASSERT_VISIT(m_tree->callbacks(), m_tree->has_parent(last_added), m_tree, last_added);
        if(m_tree->_p(last_added)->m_type == NOTYPE)
        {
    ...
    {
        _RYML_CB_ASSERT_BASIC(m_stack.m_callbacks, m_tree);
        _RYML_CB_ASSERT_BASIC(m_tree->callbacks(), m_tree->_p(m_curr->node_id)->m_val.scalar.len == 0);
