...
struct FilterResult
{
    ...
    C4_ALWAYS_INLINE csubstr get() const { RYML_ASSERT_BASIC(valid()); return str; }
    csubstr str;
};
/** Abstracts the fact that a scalar filter result may not fit in the
 * intended memory. */
...
struct FilterProcessorSrcDst
{
    ...
    C4_ALWAYS_INLINE FilterProcessorSrcDst(csubstr src_, substr dst_) noexcept
        ...
        , wpos(0)
    {
        RYML_ASSERT_BASIC(!dst.overlaps(src));
    }
    ...
    C4_ALWAYS_INLINE bool has_more_chars(size_t maxpos) const noexcept { RYML_ASSERT_BASIC(maxpos <= src.len); return rpos < maxpos; }
    ...
    C4_ALWAYS_INLINE char curr() const noexcept { RYML_ASSERT_BASIC(rpos < src.len); return src[rpos]; }
    ...
    {
        RYML_ASSERT_BASIC(rpos < src.len);
        if(wpos < dst.len)
    ...
    {
        RYML_ASSERT_BASIC(num);
        RYML_ASSERT_BASIC(rpos+num <= src.len);
        if(wpos + num <= dst.len)
    ...
    {
        RYML_ASSERT_BASIC(nw > 0);
        RYML_ASSERT_BASIC(nr > 0);
        RYML_ASSERT_BASIC(rpos+nr <= src.len);
        if(wpos+nw <= dst.len)
...
struct FilterProcessorInplaceEndExtending
{
    ...
    C4_ALWAYS_INLINE FilterProcessorInplaceEndExtending(substr src_, size_t wcap_) noexcept
        ...
        , wpos(0)
    {
        RYML_ASSERT_BASIC(wcap >= src.len);
    }
    ...
    {
        RYML_ASSERT_BASIC(rpos + 2 <= src.len);
        RYML_ASSERT_BASIC(wpos <= rpos);
        if(wpos < wcap) // respect write-capacity
            src.str[wpos] = c;
...
struct FilterProcessorInplaceMidExtending
{
    ...
    C4_NO_INLINE void translate_esc_extending(const char *C4_RESTRICT s, size_t nw, size_t nr) noexcept
    {
        ...
        else // there is overlap. move the (to-be-read) string to the right.
        {
            ...
            {
                RYML_ASSERT_BASIC(rpos+nr+excess <= src.len);
                if(wpos_next <= wcap)
                {
                    if(!unfiltered_chars)
                    {
            ...
            {
                //const size_t unw = nw > (nr + 1u) ? nw - (nr + 1u) : 0;
                RYML_ASSERT_BASIC(rpos_next <= src.len);
                ...
                RYML_ASSERT_BASIC(required_size > wcap);
                unfiltered_chars = true;
