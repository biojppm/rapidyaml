...
protected:
    ...
    {
        _RYML_CB_ASSERT_BASIC(m_stack.m_callbacks, m_curr != nullptr);
        _RYML_CB_ASSERT_BASIC(m_stack.m_callbacks, relocate_arena != nullptr);
        _RYML_CB_ASSERT_BASIC(m_stack.m_callbacks, relocate_arena_data != nullptr);
...
protected:
    ...
    void _stack_pop()
    {
        ...
        _RYML_CB_ASSERT_BASIC(m_stack.m_callbacks, m_stack.size() > 1);
...
protected:
    ...
    void _stack_relocate_to_new_arena(csubstr prev, substr curr)
    {
        ...
        _RYML_CB_ASSERT_BASIC(m_stack.m_callbacks, m_relocate_arena != nullptr);
        _RYML_CB_ASSERT_BASIC(m_stack.m_callbacks, m_relocate_arena_data != nullptr);
        m_relocate_arena(m_relocate_arena_data, prev, curr);
    }

    substr _stack_relocate_to_new_arena(csubstr s, csubstr prev, substr curr)
    {
        _RYML_CB_ASSERT_BASIC(m_stack.m_callbacks, prev.is_super(s));
        auto pos = s.str - prev.str;
        substr out = {curr.str + pos, s.len};
        _RYML_CB_ASSERT_BASIC(m_stack.m_callbacks, curr.is_super(out));
        return out;
    }
...
public:
    ...
    void check_trailing_doc_token() const
    {
        ...
        if((is_root || _has_any_(DOC)) && suspicious && !isndoc)
            _RYML_CB_ERR_PARSE(m_stack.m_callbacks, m_curr->pos, "parse error");
    }
