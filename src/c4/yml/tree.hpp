...
public:
    ...
    {
        // key cannot be empty
        RYML_ASSERT_BASIC(key.scalar.empty() == ((type & KEY) == 0));
        // key tag cannot be empty
        RYML_ASSERT_BASIC(key.tag.empty() == ((type & KEYTAG) == 0));
        // val may be empty even though VAL is set. But when VAL is not set, val must be empty
        RYML_ASSERT_BASIC(((type & VAL) != 0) || val.scalar.empty());
        // val tag cannot be empty
        RYML_ASSERT_BASIC(val.tag.empty() == ((type & VALTAG) == 0));
        return true;
    }
};
...
public:
    ...
    id_type slack() const { RYML_ASSERT_BASIC(m_cap >= m_size); return m_cap - m_size; }
...
public:
    ...
    id_type id(NodeData const* n) const
    {
        if( ! n)
            return NONE;
        _RYML_CB_ASSERT_VISIT(m_callbacks, n >= m_buf && n < m_buf + m_cap, this, NONE);
    ...
    NodeData *get(id_type node) // NOLINT(readability-make-member-function-const)
    {
        if(node == NONE)
            return nullptr;
        _RYML_CB_ASSERT_VISIT(m_callbacks, node >= 0 && node < m_cap, this, node);
        return m_buf + node;
    }
    //! get a pointer to a node's NodeData.
    //! i can be NONE, in which case a nullptr is returned.
    ...
    NodeData       * _p(id_type node)       { _RYML_CB_ASSERT_VISIT(m_callbacks, node != NONE && node >= 0 && node < m_cap, this, node); return m_buf + node; } // NOLINT(readability-make-member-function-const)
    //! An if-less form of get() that demands a valid node index.
    //! This function is implementation only; use at your own risk.
    NodeData const * _p(id_type node) const { _RYML_CB_ASSERT_VISIT(m_callbacks, node != NONE && node >= 0 && node < m_cap, this, node); return m_buf + node; }

    //! Get the id of the root node
    id_type root_id()       { if(m_cap == 0) { reserve(16); } _RYML_CB_ASSERT_VISIT(m_callbacks, m_cap > 0 && m_size > 0, this, NONE); return 0; }
    //! Get the id of the root node
    id_type root_id() const {                                 _RYML_CB_ASSERT_VISIT(m_callbacks, m_cap > 0 && m_size > 0, this, NONE); return 0; }
...
public:
    ...
    csubstr    const& key       (id_type node) const { _RYML_CB_ASSERT_VISIT(m_callbacks, has_key(node), this, node); return _p(node)->m_key.scalar; }
    csubstr    const& key_tag   (id_type node) const { _RYML_CB_ASSERT_VISIT(m_callbacks, has_key_tag(node), this, node); return _p(node)->m_key.tag; }
    csubstr    const& key_ref   (id_type node) const { _RYML_CB_ASSERT_VISIT(m_callbacks, is_key_ref(node), this, node); return _p(node)->m_key.anchor; }
    csubstr    const& key_anchor(id_type node) const { _RYML_CB_ASSERT_VISIT(m_callbacks, has_key_anchor(node), this, node); return _p(node)->m_key.anchor; }
    NodeScalar const& keysc     (id_type node) const { _RYML_CB_ASSERT_VISIT(m_callbacks, has_key(node), this, node); return _p(node)->m_key; }

    csubstr    const& val       (id_type node) const { _RYML_CB_ASSERT_VISIT(m_callbacks, has_val(node), this, node); return _p(node)->m_val.scalar; }
    csubstr    const& val_tag   (id_type node) const { _RYML_CB_ASSERT_VISIT(m_callbacks, has_val_tag(node), this, node); return _p(node)->m_val.tag; }
    csubstr    const& val_ref   (id_type node) const { _RYML_CB_ASSERT_VISIT(m_callbacks, is_val_ref(node), this, node); return _p(node)->m_val.anchor; }
    csubstr    const& val_anchor(id_type node) const { _RYML_CB_ASSERT_VISIT(m_callbacks, has_val_anchor(node), this, node); return _p(node)->m_val.anchor; }
    NodeScalar const& valsc     (id_type node) const { _RYML_CB_ASSERT_VISIT(m_callbacks, has_val(node), this, node); return _p(node)->m_val; }
...
public:
    ...
    C4_ALWAYS_INLINE bool parent_is_seq(id_type node) const { _RYML_CB_ASSERT_VISIT(m_callbacks, has_parent(node), this, node); return is_seq(_p(node)->m_parent); }
    C4_ALWAYS_INLINE bool parent_is_map(id_type node) const { _RYML_CB_ASSERT_VISIT(m_callbacks, has_parent(node), this, node); return is_map(_p(node)->m_parent); }
    ...
    C4_ALWAYS_INLINE bool key_is_null(id_type node) const { _RYML_CB_ASSERT_VISIT(m_callbacks, has_key(node), this, node); NodeData const* C4_RESTRICT n = _p(node); return !n->m_type.is_key_quoted() && (n->m_type.key_is_null() || scalar_is_null(n->m_key.scalar)); }
     ...
     * @see https://github.com/biojppm/rapidyaml/issues/413 */
    C4_ALWAYS_INLINE bool val_is_null(id_type node) const { _RYML_CB_ASSERT_VISIT(m_callbacks, has_val(node), this, node); NodeData const* C4_RESTRICT n = _p(node); return !n->m_type.is_val_quoted() && (n->m_type.val_is_null() || scalar_is_null(n->m_val.scalar)); }
...
public:
    ...
    bool is_root(id_type node) const { _RYML_CB_ASSERT_VISIT(m_callbacks, _p(node)->m_parent != NONE || node == 0, this, node); return _p(node)->m_parent == NONE; }
...
public:
    ...
    id_type num_other_siblings(id_type node) const { id_type ns = num_siblings(node); _RYML_CB_ASSERT_VISIT(m_callbacks, ns > 0, this, node); return ns-1; }
    id_type sibling_pos(id_type node, id_type sib) const { _RYML_CB_ASSERT_VISIT(m_callbacks,  ! is_root(node) || node == root_id(), this, node); return child_pos(_p(node)->m_parent, sib); }
    ...
    id_type doc(id_type i) const { id_type rid = root_id(); _RYML_CB_ASSERT_VISIT(m_callbacks, is_stream(rid), this, rid); return child(rid, i); } //!< gets the @p i document node index. requires that the root node is a stream.
...
public:
    ...
    C4_ALWAYS_INLINE void set_container_style(id_type node, NodeType_e style) { _RYML_CB_ASSERT_VISIT(m_callbacks, is_container(node), this, node); _p(node)->m_type.set_container_style(style); }
    C4_ALWAYS_INLINE void set_key_style(id_type node, NodeType_e style) { _RYML_CB_ASSERT_VISIT(m_callbacks, has_key(node), this, node); _p(node)->m_type.set_key_style(style); }
    C4_ALWAYS_INLINE void set_val_style(id_type node, NodeType_e style) { _RYML_CB_ASSERT_VISIT(m_callbacks, has_val(node), this, node); _p(node)->m_type.set_val_style(style); }
...
public:
    ...
    void set_key(id_type node, csubstr key) { _RYML_CB_ASSERT_VISIT(m_callbacks, has_key(node), this, node); _p(node)->m_key.scalar = key; }
    void set_val(id_type node, csubstr val) { _RYML_CB_ASSERT_VISIT(m_callbacks, has_val(node), this, node); _p(node)->m_val.scalar = val; }

    void set_key_tag(id_type node, csubstr tag) { _RYML_CB_ASSERT_VISIT(m_callbacks, has_key(node), this, node); _p(node)->m_key.tag = tag; _add_flags(node, KEYTAG); }
    void set_val_tag(id_type node, csubstr tag) { _RYML_CB_ASSERT_VISIT(m_callbacks, has_val(node) || is_container(node), this, node); _p(node)->m_val.tag = tag; _add_flags(node, VALTAG); }

    void set_key_anchor(id_type node, csubstr anchor) { _RYML_CB_ASSERT_VISIT(m_callbacks,  ! is_key_ref(node), this, node); _p(node)->m_key.anchor = anchor.triml('&'); _add_flags(node, KEYANCH); }
    void set_val_anchor(id_type node, csubstr anchor) { _RYML_CB_ASSERT_VISIT(m_callbacks,  ! is_val_ref(node), this, node); _p(node)->m_val.anchor = anchor.triml('&'); _add_flags(node, VALANCH); }
    void set_key_ref   (id_type node, csubstr ref   ) { _RYML_CB_ASSERT_VISIT(m_callbacks,  ! has_key_anchor(node), this, node); NodeData* C4_RESTRICT n = _p(node); n->m_key.set_ref_maybe_replacing_scalar(ref, n->m_type.has_key()); _add_flags(node, KEY|KEYREF); }
    void set_val_ref   (id_type node, csubstr ref   ) { _RYML_CB_ASSERT_VISIT(m_callbacks,  ! has_val_anchor(node), this, node); NodeData* C4_RESTRICT n = _p(node); n->m_val.set_ref_maybe_replacing_scalar(ref, n->m_type.has_val()); _add_flags(node, VAL|VALREF); }
...
public:
    ...
    {
        _RYML_CB_ASSERT_VISIT(m_callbacks, parent != NONE, this, parent);
        _RYML_CB_ASSERT_VISIT(m_callbacks, is_container(parent) || is_root(parent), this, parent);
        _RYML_CB_ASSERT_VISIT(m_callbacks, after == NONE || (_p(after)->m_parent == parent), this, parent);
...
public:
    ...
    size_t arena_slack() const { _RYML_CB_ASSERT_VISIT(m_callbacks, m_arena.len >= m_arena_pos, this, NONE); return m_arena.len - m_arena_pos; }
        ...
        -> typename std::enable_if<std::is_floating_point<T>::value, csubstr>::type
    {
        ...
        if(num > rem.len)
        {
            ...
            _RYML_CB_ASSERT_VISIT(m_callbacks, num <= rem.len, this, NONE);
        }
    ...
    substr copy_to_arena(csubstr s)
    {
        ...
        _RYML_CB_ASSERT_VISIT(m_callbacks, cp.len == s.len, this, NONE);
        _RYML_CB_ASSERT_VISIT(m_callbacks, !s.overlaps(cp), this, NONE);
    ...
    void reserve_arena(size_t arena_cap)
    {
        if(arena_cap > m_arena.len)
        {
            ...
            if(m_arena.str)
            {
                _RYML_CB_ASSERT_VISIT(m_callbacks, m_arena.len >= 0, this, NONE);
...
private:
    ...
    {
        _RYML_CB_ASSERT_VISIT(m_callbacks, m_arena_pos + sz <= m_arena.len, this, NONE);
        substr s;
    ...
    {
        _RYML_CB_ASSERT_VISIT(m_callbacks, m_arena.is_super(s), this, NONE);
        _RYML_CB_ASSERT_VISIT(m_callbacks, m_arena.sub(0, m_arena_pos).is_super(s), this, NONE);
        auto pos = (s.str - m_arena.str); // this is larger than 0 based on the assertions above
        substr r(next_arena.str + pos, s.len);
        _RYML_CB_ASSERT_VISIT(m_callbacks, r.str - next_arena.str == pos, this, NONE);
        _RYML_CB_ASSERT_VISIT(m_callbacks, next_arena.sub(0, m_arena_pos).is_super(r), this, NONE);
        return r;
    }
...
public:
    ...
    void _check_next_flags(id_type node, type_bits f)
    {
        ...
        if(f & MAP)
        {
            _RYML_CB_ASSERT_VISIT_MSG(m_callbacks, (f & SEQ) == 0, this, node, "cannot mark simultaneously as map and seq", 0);
            _RYML_CB_ASSERT_VISIT_MSG(m_callbacks, (f & VAL) == 0, this, node, "cannot mark simultaneously as map and val", 0);
            _RYML_CB_ASSERT_VISIT_MSG(m_callbacks, (o & SEQ) == 0, this, node, "cannot turn a seq into a map; clear first", 0);
            _RYML_CB_ASSERT_VISIT_MSG(m_callbacks, (o & VAL) == 0, this, node, "cannot turn a val into a map; clear first", 0);
        }
        else if(f & SEQ)
        {
            _RYML_CB_ASSERT_VISIT_MSG(m_callbacks, (f & MAP) == 0, this, node, "cannot mark simultaneously as seq and map", 0);
            _RYML_CB_ASSERT_VISIT_MSG(m_callbacks, (f & VAL) == 0, this, node, "cannot mark simultaneously as seq and val", 0);
            _RYML_CB_ASSERT_VISIT_MSG(m_callbacks, (o & MAP) == 0, this, node, "cannot turn a map into a seq; clear first", 0);
            _RYML_CB_ASSERT_VISIT_MSG(m_callbacks, (o & VAL) == 0, this, node, "cannot turn a val into a seq; clear first", 0);
        }
        if(f & KEY)
        {
            _RYML_CB_ASSERT_VISIT(m_callbacks, !is_root(node), this, node);
            auto pid = parent(node); C4_UNUSED(pid);
            _RYML_CB_ASSERT_VISIT(m_callbacks, is_map(pid), this, node);
        }
        if((f & VAL) && !is_root(node))
        {
            auto pid = parent(node); C4_UNUSED(pid);
            _RYML_CB_ASSERT_VISIT(m_callbacks, is_map(pid) || is_seq(pid), this, node);
        }
    }
    #endif
    ...
    {
        _RYML_CB_ASSERT_VISIT(m_callbacks, num_children(node) == 0, this, node);
        _RYML_CB_ASSERT_VISIT(m_callbacks, !is_seq(node) && !is_map(node), this, node);
    ...
    {
        _RYML_CB_ASSERT_VISIT(m_callbacks, num_children(node) == 0, this, node);
        _RYML_CB_ASSERT_VISIT(m_callbacks,  ! is_container(node), this, node);
        _p(node)->m_val = val;
        _add_flags(node, VAL|more_flags);
    }
    ...
    {
        _RYML_CB_ASSERT_VISIT(m_callbacks, i._check(), this, node);
        NodeData *n = _p(node);
        _RYML_CB_ASSERT_VISIT(m_callbacks, n->m_key.scalar.empty() || i.key.scalar.empty() || i.key.scalar == n->m_key.scalar, this, node);
    ...
    {
        _RYML_CB_ASSERT_VISIT(m_callbacks, is_seq(node), this, node);
