...
LocationResolver::LocationResolver(Callbacks const& cb)
    : m_filename()
    , m_src()
    , m_offsets()
    , m_size()
    ...
    , m_callbacks(that.m_callbacks)
{
    if(that.m_capacity)
    {
        ...
        _RYML_CHECK_BASIC_(m_callbacks, m_capacity == that.m_capacity);
...
LocationResolver& LocationResolver::operator=(LocationResolver const& that)
{
    if(&that != this)
    {
        ...
        if(that.m_capacity)
            _resize(that.m_capacity, false);
        _RYML_CHECK_BASIC_(m_callbacks, m_capacity >= that.m_capacity);
        _RYML_CHECK_BASIC_(m_callbacks, m_capacity >= that.m_size);
        memcpy(m_offsets, that.m_offsets, that.m_size * sizeof(size_t));
        m_size = that.m_size;
    }
    return *this;
...
{
    if(m_offsets)
        _RYML_CB_FREE(get_callbacks(), m_offsets, size_t, m_capacity);
    _clr();
}
...
void LocationResolver::set_callbacks(Callbacks const& cb)
{
    ...
    if(m_capacity)
    {
        size_t *buf = _RYML_CB_ALLOC_HINT(m_callbacks, size_t, m_capacity, m_offsets);
        if(m_size)
            memcpy(buf, m_offsets, m_size * sizeof(size_t));
        _RYML_CB_FREE(m_callbacks, m_offsets, size_t, m_capacity);
        m_offsets = buf;
    }
}
...
void LocationResolver::_resize(size_t numnewlines, bool copy)
{
    ...
    {
        size_t *buf = _RYML_CB_ALLOC_HINT(m_callbacks, size_t, numnewlines, m_offsets);
        if(m_capacity)
        {
            if(copy)
                memcpy(buf, m_offsets, m_size * sizeof(size_t));
...
{
    _RYML_ASSERT_BASIC_(m_callbacks, loc.name == m_filename);
    _RYML_ASSERT_BASIC_(m_callbacks, loc.offset < m_src.len);
...
Location LocationResolver::val_location(const char *val, size_t linear_threshold) const
{
    ...
    _RYML_ASSERT_BASIC_(m_callbacks, m_offsets != nullptr);
    _RYML_ASSERT_BASIC_(m_callbacks, m_size > 0);
    // NOTE: the pointer needs to belong to the buffer that was used to parse.
    _RYML_CHECK_BASIC_(m_callbacks, val != nullptr || m_src.str == nullptr);
    _RYML_CHECK_BASIC_(m_callbacks, (val >= m_src.begin() && val <= m_src.end()) || (m_src.str == nullptr && val == nullptr));
    ...
    _RYML_ASSERT_BASIC_(m_callbacks, lineptr >= m_offsets);
    _RYML_ASSERT_BASIC_(m_callbacks, lineptr <= m_offsets + m_size);
    _RYML_ASSERT_BASIC_(m_callbacks, *lineptr > offset);
    Location loc;
    loc.name = m_filename;
    loc.offset = offset;
    ...
    {
        _RYML_ASSERT_BASIC_(m_callbacks, *(lineptr-1) >= 1u);
        _RYML_ASSERT_BASIC_(m_callbacks, loc.col >= *(lineptr-1) - 1u);
