...
template<class T> bool read(NodeRef const& C4_RESTRICT n, T *v);
template<class T> bool read_key(NodeRef const& C4_RESTRICT n, T *v);
...
struct children_view_
{
    ...
    children_view_(tree_type *t, id_type b_, id_type e_) : tree(t), b(b_), e(e_) {}

    C4_ALWAYS_INLINE iterator begin() const { return {tree, b}; }
    C4_ALWAYS_INLINE iterator end  () const { return {tree, e}; }
};
...
public:
    ...
    template<class T>
    C4_NODISCARD bool deserialize_val(T *v) const
    {
        _C4RR();
    ...
    template<class T>
    C4_NODISCARD bool deserialize_val(T const& wrapper) const
    {
        _C4RR();
        return read((ConstImpl const&)*this, const_cast<T*>(&wrapper));
    }
    template<class T>
    C4_NODISCARD bool deserialize_key(T const& wrapper) const
    {
        _C4RR();
        return read_key((ConstImpl const&)*this, const_cast<T*>(&wrapper));
    }
    ...
    {
        _C4RR();
        ConstImpl ch = ((const Impl*)this)->find_child(name);
        if(!ch.readable())
            return false;
        ch >> *var;
        return true;
...
public:
    ...
    C4_ALWAYS_INLINE ConstNodeRef parent()                   const RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->parent(m_id)}; }              /**< Forward to @ref Tree::parent(). Node must be readable. */
    C4_ALWAYS_INLINE ConstNodeRef first_child()              const RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->first_child(m_id)}; }         /**< Forward to @ref Tree::first_child(). Node must be readable. */
    C4_ALWAYS_INLINE ConstNodeRef last_child()               const RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->last_child (m_id)}; }         /**< Forward to @ref Tree::last_child(). Node must be readable. */
    C4_ALWAYS_INLINE ConstNodeRef child(id_type pos)         const RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->child(m_id, pos)}; }          /**< Forward to @ref Tree::child(). Node must be readable. */
    C4_ALWAYS_INLINE ConstNodeRef find_child(csubstr name)   const RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->find_child(m_id, name)}; }    /**< Forward to @ref Tree::find_child(). Node must be readable. */
    C4_ALWAYS_INLINE ConstNodeRef prev_sibling()             const RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->prev_sibling(m_id)}; }        /**< Forward to @ref Tree::prev_sibling(). Node must be readable. */
    C4_ALWAYS_INLINE ConstNodeRef next_sibling()             const RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->next_sibling(m_id)}; }        /**< Forward to @ref Tree::next_sibling(). Node must be readable. */
    C4_ALWAYS_INLINE ConstNodeRef first_sibling()            const RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->first_sibling(m_id)}; }       /**< Forward to @ref Tree::first_sibling(). Node must be readable. */
    C4_ALWAYS_INLINE ConstNodeRef last_sibling ()            const RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->last_sibling(m_id)}; }        /**< Forward to @ref Tree::last_sibling(). Node must be readable. */
    C4_ALWAYS_INLINE ConstNodeRef sibling(id_type pos)       const RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->sibling(m_id, pos)}; }        /**< Forward to @ref Tree::sibling(). Node must be readable. */
    C4_ALWAYS_INLINE ConstNodeRef find_sibling(csubstr name) const RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->find_sibling(m_id, name)}; }  /**< Forward to @ref Tree::find_sibling(). Node must be readable. */
    C4_ALWAYS_INLINE ConstNodeRef ancestor_doc()             const RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->ancestor_doc(m_id)}; }        /**< Forward to @ref Tree::ancestor_doc(). Node must be readable. */
    C4_ALWAYS_INLINE ConstNodeRef doc(id_type i)             const RYML_NOEXCEPT { _RYML_ASSERT_BASIC(m_tree); return {m_tree, m_tree->doc(i)}; } /**< Forward to @ref Tree::doc(). Node must be readable. succeeds even when the node may have invalid or seed id */
...
public:
    ...
    C4_ALWAYS_INLINE const_iterator begin()  const RYML_NOEXCEPT { _C4RR(); return const_iterator(m_tree, m_tree->first_child(m_id)); } /**< get an iterator to the first child */
    C4_ALWAYS_INLINE const_iterator cbegin() const RYML_NOEXCEPT { _C4RR(); return const_iterator(m_tree, m_tree->first_child(m_id)); } /**< get an iterator to the first child */

    C4_ALWAYS_INLINE const_iterator end()  const RYML_NOEXCEPT { _C4RR(); return const_iterator(m_tree, NONE); } /**< get an iterator to after the last child */
    C4_ALWAYS_INLINE const_iterator cend() const RYML_NOEXCEPT { _C4RR(); return const_iterator(m_tree, NONE); } /**< get an iterator to after the last child */


    C4_ALWAYS_INLINE const_children_view children()  const RYML_NOEXCEPT { _C4RR(); return detail::make_children_view<const_children_view>(m_tree, m_id); } /**< get an iterable view over children */
    C4_ALWAYS_INLINE const_children_view cchildren() const RYML_NOEXCEPT { _C4RR(); return detail::make_children_view<const_children_view>(m_tree, m_id); } /**< get an iterable view over children */

    C4_ALWAYS_INLINE const_children_view siblings()  const RYML_NOEXCEPT { _C4RR(); return detail::make_siblings_view<const_children_view>(m_tree, m_id); } /** get an iterable view over all siblings (including the calling node) */
    C4_ALWAYS_INLINE const_children_view csiblings() const RYML_NOEXCEPT { _C4RR(); return detail::make_siblings_view<const_children_view>(m_tree, m_id); } /** get an iterable view over all siblings (including the calling node) */
...
public:
    ...
    void create() { _apply_seed(); } ///< if this node is in seed state, create the node in the tree

    void set_stream() { _apply_seed(); m_tree->set_stream(m_id); }
    void set_doc() { _apply_seed(); m_tree->set_doc(m_id); }

    void set_key(csubstr key) { _apply_seed(); m_tree->set_key(m_id, key); }
    void set_key(csubstr key, NodeType more_flags) { _apply_seed(); m_tree->set_key(m_id, key, more_flags); }

    void set_val(csubstr val) { _apply_seed(); m_tree->set_val(m_id, val); }
    void set_val(csubstr val, NodeType more_flags) { _apply_seed(); m_tree->set_val(m_id, val, more_flags); }

    void set_seq() { _apply_seed(); m_tree->set_seq(m_id); }
    void set_seq(NodeType more_flags) { _apply_seed(); m_tree->set_seq(m_id, more_flags); }

    void set_map() { _apply_seed(); m_tree->set_map(m_id); }
    void set_map(NodeType more_flags) { _apply_seed(); m_tree->set_map(m_id, more_flags); }
...
public:
    ...
    /** serialize a variable, then assign the result to the node */
    template<class T>
    void set_val_serialized(T const& C4_RESTRICT v)
    {
        _apply_seed();
        write(*this, v);
    }
    ...
    /** serialize a csubstr, then assign the result to the node. This
     * function is needed for overload disambiguation. */
    NodeRef& operator<< (csubstr const& C4_RESTRICT v)
    {
        _apply_seed();
        set_val_serialized(v);
        return *this;
    }
    ...
    {
        _apply_seed();
        write_key(*this, std::forward<T>(v.k));
        return *this;
    }
    /** serialize a csubstr, then assign the result to the node's
     * key. This function is needed for overload disambiguation. */
    NodeRef& operator<< (Key<csubstr> const& C4_RESTRICT v)
    {
        _apply_seed();
...
public:
    ...
    C4_ALWAYS_INLINE NodeRef      parent()                         RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->parent(m_id)}; }              /**< Forward to @ref Tree::parent(). Node must be readable. */
    C4_ALWAYS_INLINE ConstNodeRef parent()                   const RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->parent(m_id)}; }              /**< Forward to @ref Tree::parent(). Node must be readable. */

    C4_ALWAYS_INLINE NodeRef      first_child()                    RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->first_child(m_id)}; }         /**< Forward to @ref Tree::first_child(). Node must be readable. */
    C4_ALWAYS_INLINE ConstNodeRef first_child()              const RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->first_child(m_id)}; }         /**< Forward to @ref Tree::first_child(). Node must be readable. */

    C4_ALWAYS_INLINE NodeRef      last_child()                     RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->last_child(m_id)}; }          /**< Forward to @ref Tree::last_child(). Node must be readable. */
    C4_ALWAYS_INLINE ConstNodeRef last_child()               const RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->last_child (m_id)}; }         /**< Forward to @ref Tree::last_child(). Node must be readable. */

    C4_ALWAYS_INLINE NodeRef      child(id_type pos)               RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->child(m_id, pos)}; }          /**< Forward to @ref Tree::child(). Node must be readable. */
    C4_ALWAYS_INLINE ConstNodeRef child(id_type pos)         const RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->child(m_id, pos)}; }          /**< Forward to @ref Tree::child(). Node must be readable. */

    C4_ALWAYS_INLINE NodeRef      find_child(csubstr name)         RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->find_child(m_id, name)}; }    /**< Forward to @ref Tree::find_child(). Node must be readable. */
    C4_ALWAYS_INLINE ConstNodeRef find_child(csubstr name)   const RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->find_child(m_id, name)}; }    /**< Forward to @ref Tree::find_child(). Node must be readable. */

    C4_ALWAYS_INLINE NodeRef      prev_sibling()                   RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->prev_sibling(m_id)}; }        /**< Forward to @ref Tree::prev_sibling(). Node must be readable. */
    C4_ALWAYS_INLINE ConstNodeRef prev_sibling()             const RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->prev_sibling(m_id)}; }        /**< Forward to @ref Tree::prev_sibling(). Node must be readable. */

    C4_ALWAYS_INLINE NodeRef      next_sibling()                   RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->next_sibling(m_id)}; }        /**< Forward to @ref Tree::next_sibling(). Node must be readable. */
    C4_ALWAYS_INLINE ConstNodeRef next_sibling()             const RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->next_sibling(m_id)}; }        /**< Forward to @ref Tree::next_sibling(). Node must be readable. */

    C4_ALWAYS_INLINE NodeRef      first_sibling()                  RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->first_sibling(m_id)}; }       /**< Forward to @ref Tree::first_sibling(). Node must be readable. */
    C4_ALWAYS_INLINE ConstNodeRef first_sibling()            const RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->first_sibling(m_id)}; }       /**< Forward to @ref Tree::first_sibling(). Node must be readable. */

    C4_ALWAYS_INLINE NodeRef      last_sibling()                   RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->last_sibling(m_id)}; }        /**< Forward to @ref Tree::last_sibling(). Node must be readable. */
    C4_ALWAYS_INLINE ConstNodeRef last_sibling()             const RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->last_sibling(m_id)}; }        /**< Forward to @ref Tree::last_sibling(). Node must be readable. */

    C4_ALWAYS_INLINE NodeRef      sibling(id_type pos)             RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->sibling(m_id, pos)}; }        /**< Forward to @ref Tree::sibling(). Node must be readable. */
    C4_ALWAYS_INLINE ConstNodeRef sibling(id_type pos)       const RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->sibling(m_id, pos)}; }        /**< Forward to @ref Tree::sibling(). Node must be readable. */

    C4_ALWAYS_INLINE NodeRef      find_sibling(csubstr name)       RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->find_sibling(m_id, name)}; }  /**< Forward to @ref Tree::find_sibling(). Node must be readable. */
    C4_ALWAYS_INLINE ConstNodeRef find_sibling(csubstr name) const RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->find_sibling(m_id, name)}; }  /**< Forward to @ref Tree::find_sibling(). Node must be readable. */

    C4_ALWAYS_INLINE NodeRef      ancestor_doc()                   RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->ancestor_doc(m_id)}; }        /**< Forward to @ref Tree::ancestor_doc(). Node must be readable. */
    C4_ALWAYS_INLINE ConstNodeRef ancestor_doc()             const RYML_NOEXCEPT { _C4RR(); return {m_tree, m_tree->ancestor_doc(m_id)}; }        /**< Forward to @ref Tree::ancestor_doc(). Node must be readable. */

    C4_ALWAYS_INLINE NodeRef      doc(id_type i)                   RYML_NOEXCEPT { _RYML_ASSERT_BASIC(m_tree); return {m_tree, m_tree->doc(i)}; } /**< Forward to @ref Tree::doc(). Node must be readable. */
    C4_ALWAYS_INLINE ConstNodeRef doc(id_type i)             const RYML_NOEXCEPT { _RYML_ASSERT_BASIC(m_tree); return {m_tree, m_tree->doc(i)}; } /**< Forward to @ref Tree::doc(). Node must be readable. succeeds even when the node may have invalid or seed id */
...
public:
    ...
    C4_ALWAYS_INLINE       iterator begin()        RYML_NOEXCEPT { _C4RR(); return iterator(m_tree, m_tree->first_child(m_id)); }       /**< get a mutable iterator to the first child. NOT AVAILABLE for ConstNodeRef. */
    C4_ALWAYS_INLINE const_iterator begin()  const RYML_NOEXCEPT { _C4RR(); return const_iterator(m_tree, m_tree->first_child(m_id)); } /**< get an iterator to the first child */
    C4_ALWAYS_INLINE const_iterator cbegin() const RYML_NOEXCEPT { _C4RR(); return const_iterator(m_tree, m_tree->first_child(m_id)); } /**< get an iterator to the first child */

    C4_ALWAYS_INLINE       iterator end()        RYML_NOEXCEPT { _C4RR(); return iterator(m_tree, NONE); }       /**< get an iterator to after the last child. NOT AVAILABLE for ConstNodeRef. */
    C4_ALWAYS_INLINE const_iterator end()  const RYML_NOEXCEPT { _C4RR(); return const_iterator(m_tree, NONE); } /**< get an iterator to after the last child */
    C4_ALWAYS_INLINE const_iterator cend() const RYML_NOEXCEPT { _C4RR(); return const_iterator(m_tree, NONE); } /**< get an iterator to after the last child */


    C4_ALWAYS_INLINE       children_view children()        RYML_NOEXCEPT { _C4RR(); return detail::make_children_view<children_view>(m_tree, m_id); }       /**< get an iterable view over children */
    C4_ALWAYS_INLINE const_children_view children()  const RYML_NOEXCEPT { _C4RR(); return detail::make_children_view<const_children_view>(m_tree, m_id); } /**< get an iterable view over children */
    C4_ALWAYS_INLINE const_children_view cchildren() const RYML_NOEXCEPT { _C4RR(); return detail::make_children_view<const_children_view>(m_tree, m_id); } /**< get an iterable view over children */

    C4_ALWAYS_INLINE       children_view siblings()        RYML_NOEXCEPT { _C4RR(); return detail::make_siblings_view<children_view>(m_tree, m_id); }       /** get an iterable view over all siblings (including the calling node) */
    C4_ALWAYS_INLINE const_children_view siblings()  const RYML_NOEXCEPT { _C4RR(); return detail::make_siblings_view<const_children_view>(m_tree, m_id); } /** get an iterable view over all siblings (including the calling node) */
    C4_ALWAYS_INLINE const_children_view csiblings() const RYML_NOEXCEPT { _C4RR(); return detail::make_siblings_view<const_children_view>(m_tree, m_id); } /** get an iterable view over all siblings (including the calling node) */
    ...
    /** visit every child node calling fn(node) */
    template<class Visitor>
    bool visit(Visitor fn, id_type indentation_level=0, bool skip_root=true) RYML_NOEXCEPT
    {
        _C4RR();
        return detail::_visit(this, fn, indentation_level, skip_root);
    }
    ...
    /** visit every child node calling fn(node, level) */
    template<class Visitor>
    bool visit_stacked(Visitor fn, id_type indentation_level=0, bool skip_root=true) RYML_NOEXCEPT
    {
        _C4RR();
        return detail::_visit_stacked(this, fn, indentation_level, skip_root);
    }
...
namespace detail {
inline void check_node_readable(NodeRef const& C4_RESTRICT n)
{
    if(C4_UNLIKELY(!n.tree()))
        _RYML_ERR_BASIC("invalid node");
    else if(C4_UNLIKELY(!n.readable()))
        _RYML_ERR_VISIT_(n.tree()->callbacks(), n.tree(), n.id(), "invalid node");
}
inline void check_node_readable(ConstNodeRef const& C4_RESTRICT n)
{
    if(C4_UNLIKELY(!n.tree()))
        _RYML_ERR_BASIC("invalid node");
    else if(C4_UNLIKELY(!n.readable()))
...
{
    write(n->tree(), n->id(), v); // defer to the tree implementation
}

template<class T>
C4_ALWAYS_INLINE void write_key(NodeRef *n, T const& v)
{
    write_key(n->tree(), n->id(), v); // defer to the tree implementation
}

template<class T>
C4_ALWAYS_INLINE void write(NodeRef &n, T const& v)
{
    write(&n, v); // forward to the ptr version
}

template<class T>
C4_ALWAYS_INLINE void write_key(NodeRef &n, T const& v)
{
    write_key(&n, v); // forward to the ptr version
}

template<class T>
C4_ALWAYS_INLINE bool read(ConstNodeRef const& C4_RESTRICT n, T *v)
{
    detail::check_node_readable(n);
...
template<class T>
C4_ALWAYS_INLINE bool read_key(ConstNodeRef const& C4_RESTRICT n, T *v)
{
    detail::check_node_readable(n);
    return read_key(n.m_tree, n.m_id, v);
}
template<class T>
C4_ALWAYS_INLINE bool read_key(ConstNodeRef const& C4_RESTRICT n, T const& wrapper)
{
    detail::check_node_readable(n);
    return read_key(n.m_tree, n.m_id, wrapper);
}

template<class T>
C4_ALWAYS_INLINE bool read(NodeRef const& C4_RESTRICT n, T *v)
{
    return read(ConstNodeRef(n), v);
}
template<class T>
C4_ALWAYS_INLINE bool read(NodeRef const& C4_RESTRICT n, T const& wrapper)
{
    return read(ConstNodeRef(n), wrapper);
}
template<class T>
C4_ALWAYS_INLINE bool read_key(NodeRef const& C4_RESTRICT n, T *v)
{
    return read_key(ConstNodeRef(n), v);
}
template<class T>
C4_ALWAYS_INLINE bool read_key(NodeRef const& C4_RESTRICT n, T const& wrapper)
{
    return read_key(ConstNodeRef(n), wrapper);
}
