...
struct child_iterator
{
    ...
    child_iterator& operator++ () { _RYML_CB_ASSERT_VISIT(m_tree->m_callbacks, m_child_id != NONE, m_tree, NONE); m_child_id = m_tree->next_sibling(m_child_id); return *this; }
    child_iterator& operator-- () { _RYML_CB_ASSERT_VISIT(m_tree->m_callbacks, m_child_id != NONE, m_tree, NONE); m_child_id = m_tree->prev_sibling(m_child_id); return *this; }

    NodeRefType operator*  () const { return NodeRefType(m_tree, m_child_id); }
    NodeRefType operator-> () const { return NodeRefType(m_tree, m_child_id); }

    bool operator!= (child_iterator that) const { RYML_ASSERT_VISIT(m_tree == that.m_tree, m_tree, NONE); return m_child_id != that.m_child_id; }
    bool operator== (child_iterator that) const { RYML_ASSERT_VISIT(m_tree == that.m_tree, m_tree, NONE); return m_child_id == that.m_child_id; }
};
...
struct RoNodeMethods
{
    ...
    #define _C4RR()                                       \
        RYML_ASSERT_BASIC(tree_ != nullptr);              \
        _RYML_CB_ASSERT_VISIT(tree_->m_callbacks, id_ != NONE, tree_, id_); \
        _RYML_CB_ASSERT_VISIT(tree_->m_callbacks, (((Impl const* C4_RESTRICT)this)->readable()), tree_, id_)
...
public:
    ...
    template<class U=Impl>
    C4_ALWAYS_INLINE auto doc(id_type i) RYML_NOEXCEPT -> _C4_IF_MUTABLE(Impl) { RYML_ASSERT_BASIC(tree_); return {tree__, tree__->doc(i)}; } /**< Forward to @ref Tree::doc(). Node must be readable. */
    C4_ALWAYS_INLINE ConstImpl doc(id_type i) const RYML_NOEXCEPT { RYML_ASSERT_BASIC(tree_); return {tree_, tree_->doc(i)}; }                /**< Forward to @ref Tree::doc(). Node must be readable. succeeds even when the node may have invalid or seed id */
    ...
    C4_ALWAYS_INLINE id_type child_pos(ConstImpl const& n) const RYML_NOEXCEPT { _C4RR(); _RYML_CB_ASSERT_VISIT(tree_->m_callbacks, n.readable(), n.tree(), n.id()); return tree_->child_pos(id_, n.m_id); } /**< O(num_children). Forward to @ref Tree::child_pos(). */
    C4_ALWAYS_INLINE id_type sibling_pos(ConstImpl const& n) const RYML_NOEXCEPT { _C4RR(); _RYML_CB_ASSERT_VISIT(tree_->callbacks(), n.readable(), n.tree(), n.id()); return tree_->child_pos(tree_->parent(id_), n.m_id); } /**< O(num_siblings). Forward to @ref Tree::sibling_pos(). */
...
public:
    ...
    C4_ALWAYS_INLINE ConstImpl operator[] (csubstr key) const RYML_NOEXCEPT
    {
        ...
        _RYML_CB_ASSERT_VISIT(tree_->m_callbacks, ch != NONE, tree_, id_);
        return {tree_, ch};
    }
...
public:
    ...
    {
        RYML_CHECK_BASIC(tree_ != nullptr);
        _RYML_CB_CHECK_VISIT(tree_->m_callbacks, (id_ >= 0 && id_ < tree_->capacity()), tree_, id_);
        _RYML_CB_CHECK_VISIT(tree_->m_callbacks, ((Impl const*)this)->readable(), tree_, id_);
        _RYML_CB_CHECK_VISIT(tree_->m_callbacks, tree_->is_map(id_), tree_, id_);
        id_type ch = tree__->find_child(id__, key);
        return ch != NONE ? Impl(tree__, ch) : Impl(tree__, id__, key);
    }
    ...
    C4_ALWAYS_INLINE auto at(id_type pos) -> _C4_IF_MUTABLE(Impl)
    {
        ...
        _RYML_CB_CHECK_VISIT(tree_->m_callbacks, (id_ >= 0 && id_ < cap), tree_, id_);
        _RYML_CB_CHECK_VISIT(tree_->m_callbacks, (pos >= 0 && pos < cap), tree_, id_);
        _RYML_CB_CHECK_VISIT(tree_->m_callbacks, ((Impl const*)this)->readable(), tree_, id_);
        _RYML_CB_CHECK_VISIT(tree_->m_callbacks, tree_->is_container(id_), tree_, id_);
        id_type ch = tree__->child(id__, pos);
        return ch != NONE ? Impl(tree__, ch) : Impl(tree__, id__, pos);
    }
    ...
    ConstImpl at(csubstr key) const
    {
        ...
        _RYML_CB_CHECK_VISIT(tree_->m_callbacks, tree_->is_map(id_), tree_, id_);
        id_type ch = tree_->find_child(id_, key);
        _RYML_CB_CHECK_VISIT(tree_->m_callbacks, ch != NONE, tree_, id_);
        return {tree_, ch};
    }
...
public:
    ...
    ConstImpl const& operator>> (T &v) const
    {
        ...
        if( ! read((ConstImpl const&)*this, &v))
            _RYML_CB_ERR_VISIT(tree_->m_callbacks, tree_, id_, "could not deserialize value");
    ...
    ConstImpl const& operator>> (Key<T> v) const
    {
        ...
        if( ! readkey((ConstImpl const&)*this, &v.k))
            _RYML_CB_ERR_VISIT(tree_->m_callbacks, tree_, id_, "could not deserialize key");
        return *((ConstImpl const*)this);
    }
...
public:
    ...
    RYML_DEPRECATED("use (this->val() == s)") bool operator== (csubstr s) const RYML_NOEXCEPT { RYML_ASSERT_BASIC(m_tree); _RYML_CB_ASSERT_VISIT(m_tree->m_callbacks, m_id != NONE, m_tree, NONE); return m_tree->val(m_id) == s; }
    RYML_DEPRECATED("use (this->val() != s)") bool operator!= (csubstr s) const RYML_NOEXCEPT { RYML_ASSERT_BASIC(m_tree); _RYML_CB_ASSERT_VISIT(m_tree->m_callbacks, m_id != NONE, m_tree, NONE); return m_tree->val(m_id) != s; }
    /** @endcond */
...
private:
    ...
    #define _C4RR()                                                     \
        RYML_ASSERT_BASIC(m_tree != nullptr);                           \
        _RYML_CB_ASSERT_VISIT(m_tree->m_callbacks, m_id != NONE && !is_seed(), m_tree, m_id)
    // require id: a helper macro, undefined at the end
    #define _C4RID()                                                        \
        RYML_ASSERT_BASIC(m_tree != nullptr);                                     \
        _RYML_CB_ASSERT_VISIT(m_tree->m_callbacks, m_id != NONE, m_tree, m_id)
...
public:
    ...
    RYML_DEPRECATED("use `this->val() == s`") bool operator== (csubstr s) const { _C4RR(); _RYML_CB_ASSERT_VISIT(m_tree->m_callbacks, has_val(), m_tree, m_id); return m_tree->val(m_id) == s; }
    RYML_DEPRECATED("use `this->val() != s`") bool operator!= (csubstr s) const { _C4RR(); _RYML_CB_ASSERT_VISIT(m_tree->m_callbacks, has_val(), m_tree, m_id); return m_tree->val(m_id) != s; }
    /** @endcond */
...
public:
    ...
    {
        RYML_ASSERT_BASIC(m_tree); // no need for valid or readable
    ...
    NodeRef& operator<< (csubstr s)
    {
        ...
        _apply_seed();
        write(this, s);
        _RYML_CB_ASSERT_VISIT(m_tree->m_callbacks, val() == s, m_tree, m_id);
        return *this;
    }
...
private:
    ...
    void _apply_seed()
    {
        ...
        {
            _RYML_CB_ASSERT_VISIT(m_tree->m_callbacks, (size_t)m_tree->num_children(m_id) == m_seed.len, m_tree, m_id);
            m_id = m_tree->append_child(m_id);
            m_seed.str = nullptr;
            m_seed.len = (size_t)NONE;
        }
        ...
        {
            _RYML_CB_ASSERT_VISIT(m_tree->m_callbacks, readable(), m_tree, m_id);
        }
    }
...
public:
    ...
    {
        _C4RR();
        _RYML_CB_ASSERT_VISIT(m_tree->m_callbacks, after.m_tree == m_tree, m_tree, m_id);
...
public:
    ...
    {
        _C4RR();
        _RYML_CB_ASSERT_VISIT(m_tree->m_callbacks, has_child(child), m_tree, m_id);
        _RYML_CB_ASSERT_VISIT(m_tree->m_callbacks, child.parent().id() == id(), m_tree, m_id);
    ...
    {
        _C4RR();
        _RYML_CB_ASSERT_VISIT(m_tree->m_callbacks, pos >= 0 && pos < num_children(), m_tree, m_id);
        id_type child = m_tree->child(m_id, pos);
        _RYML_CB_ASSERT_VISIT(m_tree->m_callbacks, child != NONE, m_tree, m_id);
        m_tree->remove(child);
    }
...
public:
    ...
    {
        _C4RR();
        _RYML_CB_ASSERT_VISIT(m_tree->m_callbacks, m_tree == after.m_tree || after.m_id == NONE, m_tree, m_id);
    ...
    {
        _C4RR();
        _RYML_CB_ASSERT_VISIT(m_tree->m_callbacks, parent.m_tree == after.m_tree || after.m_id == NONE, m_tree, m_id);
        if(parent.m_tree == m_tree)
        {
    ...
    {
        _C4RR();
        _RYML_CB_ASSERT_VISIT(m_tree->m_callbacks, parent.m_tree == after.m_tree, m_tree, m_id);
        if(parent.m_tree == m_tree)
        {
