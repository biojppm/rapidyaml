...
void parse_in_place(Parser *parser, csubstr filename, substr yaml, NodeRef node            ) { _RYML_CHECK_BASIC(!node.invalid()); node.create(); parse_in_place(parser, filename, yaml, node.tree(), node.id()); }
void parse_in_place(Parser *parser,                   substr yaml, NodeRef node            ) { _RYML_CHECK_BASIC(!node.invalid()); node.create(); parse_in_place(parser, {}      , yaml, node.tree(), node.id()); }
...
void parse_in_place(csubstr filename, substr yaml, NodeRef node            , ParserOptions const& opts) { _RYML_CHECK_BASIC(!node.invalid()); node.create(); Parser::handler_type event_handler(node.tree()->callbacks()); Parser parser(&event_handler, opts); parse_in_place(&parser, filename, yaml, node.tree(), node.id()); }
void parse_in_place(                  substr yaml, NodeRef node            , ParserOptions const& opts) { _RYML_CHECK_BASIC(!node.invalid()); node.create(); Parser::handler_type event_handler(node.tree()->callbacks()); Parser parser(&event_handler, opts); parse_in_place(&parser, {}      , yaml, node.tree(), node.id()); }
...
void parse_json_in_place(Parser *parser, csubstr filename, substr json, NodeRef node            ) { _RYML_CHECK_BASIC(!node.invalid()); node.create(); parse_json_in_place(parser, filename, json, node.tree(), node.id()); }
void parse_json_in_place(Parser *parser,                   substr json, NodeRef node            ) { _RYML_CHECK_BASIC(!node.invalid()); node.create(); parse_json_in_place(parser, {}      , json, node.tree(), node.id()); }
...
void parse_json_in_place(csubstr filename, substr json, NodeRef node            , ParserOptions const& opts) { _RYML_CHECK_BASIC(!node.invalid()); node.create(); Parser::handler_type event_handler(node.tree()->callbacks()); Parser parser(&event_handler, opts); parse_json_in_place(&parser, filename, json, node.tree(), node.id()); }
void parse_json_in_place(                  substr json, NodeRef node            , ParserOptions const& opts) { _RYML_CHECK_BASIC(!node.invalid()); node.create(); Parser::handler_type event_handler(node.tree()->callbacks()); Parser parser(&event_handler, opts); parse_json_in_place(&parser, {}      , json, node.tree(), node.id()); }
...
void parse_in_arena(Parser *parser, csubstr filename, csubstr yaml, NodeRef node            ) { _RYML_CHECK_BASIC(!node.invalid()); node.create(); substr src = node.tree()->copy_to_arena(yaml); parse_in_place(parser, filename, src, node.tree(), node.id()); }
void parse_in_arena(Parser *parser,                   csubstr yaml, NodeRef node            ) { _RYML_CHECK_BASIC(!node.invalid()); node.create(); substr src = node.tree()->copy_to_arena(yaml); parse_in_place(parser, {}      , src, node.tree(), node.id()); }
...
void parse_in_arena(csubstr filename, csubstr yaml, NodeRef node            , ParserOptions const& opts) { _RYML_CHECK_BASIC(!node.invalid()); node.create(); Parser::handler_type event_handler(node.tree()->callbacks()); Parser parser(&event_handler, opts); substr src = node.tree()->copy_to_arena(yaml); parse_in_place(&parser, filename, src, node.tree(), node.id()); }
void parse_in_arena(                  csubstr yaml, NodeRef node            , ParserOptions const& opts) { _RYML_CHECK_BASIC(!node.invalid()); node.create(); Parser::handler_type event_handler(node.tree()->callbacks()); Parser parser(&event_handler, opts); substr src = node.tree()->copy_to_arena(yaml); parse_in_place(&parser, {}      , src, node.tree(), node.id()); }
...
void parse_json_in_arena(Parser *parser, csubstr filename, csubstr json, NodeRef node            ) { _RYML_CHECK_BASIC(!node.invalid()); node.create(); substr src = node.tree()->copy_to_arena(json); parse_json_in_place(parser, filename, src, node.tree(), node.id()); }
void parse_json_in_arena(Parser *parser,                   csubstr json, NodeRef node            ) { _RYML_CHECK_BASIC(!node.invalid()); node.create(); substr src = node.tree()->copy_to_arena(json); parse_json_in_place(parser, {}      , src, node.tree(), node.id()); }
...
void parse_json_in_arena(csubstr filename, csubstr json, NodeRef node            , ParserOptions const& opts) { _RYML_CHECK_BASIC(!node.invalid()); node.create(); Parser::handler_type event_handler(node.tree()->callbacks()); Parser parser(&event_handler, opts); substr src = node.tree()->copy_to_arena(json); parse_json_in_place(&parser, filename, src, node.tree(), node.id()); }
void parse_json_in_arena(                  csubstr json, NodeRef node            , ParserOptions const& opts) { _RYML_CHECK_BASIC(!node.invalid()); node.create(); Parser::handler_type event_handler(node.tree()->callbacks()); Parser parser(&event_handler, opts); substr src = node.tree()->copy_to_arena(json); parse_json_in_place(&parser, {}      , src, node.tree(), node.id()); }
