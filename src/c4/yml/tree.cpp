...
{
    _RYML_CB_ASSERT_VISIT(m_callbacks, id != NONE && id >= 0 && id < m_cap, this, id);
...
void Tree::_free()
{
    if(m_buf)
    {
        _RYML_CB_ASSERT_VISIT(m_callbacks, m_cap > 0, this, NONE);
...
{
    _RYML_CB_ASSERT_VISIT(m_callbacks, m_buf == nullptr, this, NONE);
    _RYML_CB_ASSERT_VISIT(m_callbacks, m_arena.str == nullptr, this, NONE);
    _RYML_CB_ASSERT_VISIT(m_callbacks, m_arena.len == 0, this, NONE);
    if(that.m_cap)
    {
    ...
    if(that.m_arena.str)
    {
        _RYML_CB_ASSERT_VISIT(m_callbacks, that.m_arena.len > 0, this, NONE);
        substr arena;
...
{
    _RYML_CB_ASSERT_VISIT(m_callbacks, next_arena.not_empty(), this, NONE);
    _RYML_CB_ASSERT_VISIT(m_callbacks, next_arena.len >= m_arena.len, this, NONE);
    if(m_arena_pos)
...
void Tree::reserve(id_type cap)
{
    if(cap > m_cap)
    {
        ...
        {
            _RYML_CB_ASSERT_VISIT(m_callbacks, m_buf != nullptr, this, NONE);
            _RYML_CB_ASSERT_VISIT(m_callbacks, m_free_tail != NONE, this, NONE);
        ...
        _RYML_CB_ASSERT_VISIT(m_callbacks, m_free_head == NONE || (m_free_head >= 0 && m_free_head < cap), this, NONE);
        _RYML_CB_ASSERT_VISIT(m_callbacks, m_free_tail == NONE || (m_free_tail >= 0 && m_free_tail < cap), this, NONE);
...
{
    id_type r = _claim();
    _RYML_CB_ASSERT_VISIT(m_callbacks, r == 0, this, r);
...
void Tree::_clear_range(id_type first, id_type num)
{
    ...
    _RYML_CB_ASSERT_VISIT(m_callbacks, first >= 0 && first + num <= m_cap, this, first);
...
{
    _RYML_CB_ASSERT_VISIT(m_callbacks, i >= 0 && i < m_cap, this, i);
...
id_type Tree::_claim()
{
    if(m_free_head == NONE || m_buf == nullptr)
    {
        ...
        sz = sz ? sz : 16;
        reserve(sz);
        _RYML_CB_ASSERT_VISIT(m_callbacks, m_free_head != NONE, this, NONE);
    }

    _RYML_CB_ASSERT_VISIT(m_callbacks, m_size < m_cap, this, NONE);
    _RYML_CB_ASSERT_VISIT(m_callbacks, m_free_head >= 0 && m_free_head < m_cap, this, NONE);
...
{
    _RYML_CB_ASSERT_VISIT(m_callbacks, ichild >= 0 && ichild < m_cap, this, ichild);
    _RYML_CB_ASSERT_VISIT(m_callbacks, iparent == NONE || (iparent >= 0 && iparent < m_cap), this, iparent);
    _RYML_CB_ASSERT_VISIT(m_callbacks, iprev_sibling == NONE || (iprev_sibling >= 0 && iprev_sibling < m_cap), this, iprev_sibling);
    ...
    if(iparent == NONE)
    {
        _RYML_CB_ASSERT_VISIT(m_callbacks, ichild == 0, this, ichild);
        _RYML_CB_ASSERT_VISIT(m_callbacks, iprev_sibling == NONE, this, iprev_sibling);
    }
    ...
    if(psib)
    {
        _RYML_CB_ASSERT_VISIT(m_callbacks, next_sibling(iprev_sibling) == id(nsib), this, iprev_sibling);
        child->m_prev_sibling = id(psib);
        psib->m_next_sibling = id(child);
        _RYML_CB_ASSERT_VISIT(m_callbacks, psib->m_prev_sibling != psib->m_next_sibling || psib->m_prev_sibling == NONE, this, iprev_sibling);
    }

    if(nsib)
    {
        _RYML_CB_ASSERT_VISIT(m_callbacks, prev_sibling(inext_sibling) == id(psib), this, inext_sibling);
        child->m_next_sibling = id(nsib);
        nsib->m_prev_sibling = id(child);
        _RYML_CB_ASSERT_VISIT(m_callbacks, nsib->m_prev_sibling != nsib->m_next_sibling || nsib->m_prev_sibling == NONE, this, inext_sibling);
    }

    if(parent->m_first_child == NONE)
    {
        _RYML_CB_ASSERT_VISIT(m_callbacks, parent->m_last_child == NONE, this, parent->m_last_child);
...
{
    _RYML_CB_ASSERT_VISIT(m_callbacks, (parent(n_) != NONE) || type(n_) == NOTYPE, this, n_);
    _RYML_CB_ASSERT_VISIT(m_callbacks, (parent(m_) != NONE) || type(m_) == NOTYPE, this, m_);
    NodeType tn = type(n_);
    NodeType tm = type(m_);
...
void Tree::_swap_hierarchy(id_type ia, id_type ib)
{
    ...
    else
    {
        ...
        {
            _RYML_CB_ASSERT_VISIT(m_callbacks, b.m_prev_sibling == ia, this, ia);
            if(a.m_prev_sibling != NONE)
            {
                _RYML_CB_ASSERT_VISIT(m_callbacks, a.m_prev_sibling != ib, this, ib);
                _p(a.m_prev_sibling)->m_next_sibling = ib;
            }
            if(b.m_next_sibling != NONE)
            {
                _RYML_CB_ASSERT_VISIT(m_callbacks, b.m_next_sibling != ia, this, ia);
                _p(b.m_next_sibling)->m_prev_sibling = ia;
            }
        ...
        else if(a.m_prev_sibling == ib) // m will go after n
        {
            ...
            {
                _RYML_CB_ASSERT_VISIT(m_callbacks, a.m_next_sibling != ib, this, ib);
                _p(a.m_next_sibling)->m_prev_sibling = ib;
            }
    ...
    _RYML_CB_ASSERT_VISIT(m_callbacks, a.m_next_sibling != ia, this, ia);
    _RYML_CB_ASSERT_VISIT(m_callbacks, a.m_prev_sibling != ia, this, ia);
    _RYML_CB_ASSERT_VISIT(m_callbacks, b.m_next_sibling != ib, this, ib);
    _RYML_CB_ASSERT_VISIT(m_callbacks, b.m_prev_sibling != ib, this, ib);
...
{
    _RYML_CB_ASSERT_VISIT(m_callbacks, node != NONE, this, node);
    _RYML_CB_ASSERT_VISIT(m_callbacks, node != after, this, node);
    _RYML_CB_ASSERT_VISIT(m_callbacks,  ! is_root(node), this, node);
    _RYML_CB_ASSERT_VISIT(m_callbacks, (after == NONE) || (has_sibling(node, after) && has_sibling(after, node)), this, node);
...
void Tree::move(id_type node, id_type new_parent, id_type after)
{
    _RYML_CB_ASSERT_VISIT(m_callbacks, node != NONE, this, node);
    _RYML_CB_ASSERT_VISIT(m_callbacks, node != after, this, node);
    _RYML_CB_ASSERT_VISIT(m_callbacks, new_parent != NONE, this, new_parent);
    _RYML_CB_ASSERT_VISIT(m_callbacks, new_parent != node, this, new_parent);
    _RYML_CB_ASSERT_VISIT(m_callbacks, new_parent != after, this, new_parent);
    _RYML_CB_ASSERT_VISIT(m_callbacks,  ! is_root(node), this, node);
...
{
    _RYML_CB_ASSERT_VISIT(m_callbacks, src != nullptr, this, new_parent);
    _RYML_CB_ASSERT_VISIT(m_callbacks, node != NONE, this, new_parent);
    _RYML_CB_ASSERT_VISIT(m_callbacks, new_parent != NONE, this, new_parent);
    _RYML_CB_ASSERT_VISIT(m_callbacks, new_parent != after, this, new_parent);
...
void Tree::set_root_as_stream()
{
    ...
    _RYML_CB_ASSERT_VISIT(m_callbacks, !has_key(root), this, root);
    id_type next_doc = append_child(root);
    _copy_props_wo_key(next_doc, root);
...
{
    _RYML_CB_ASSERT_VISIT(m_callbacks, get(node) != nullptr, this, node);
    id_type ich = get(node)->m_first_child;
    while(ich != NONE)
    {
        remove_children(ich);
        _RYML_CB_ASSERT_VISIT(m_callbacks, get(ich) != nullptr, this, node);
...
{
    _RYML_CB_ASSERT_VISIT(m_callbacks, type.is_val() || type.is_map() || type.is_seq(), this, node);
    _RYML_CB_ASSERT_VISIT(m_callbacks, type.is_val() + type.is_map() + type.is_seq() == 1, this, node);
    _RYML_CB_ASSERT_VISIT(m_callbacks, type.has_key() == has_key(node) || (has_key(node) && !type.has_key()), this, node);
    NodeData *d = _p(node);
...
{
    _RYML_CB_ASSERT_VISIT(m_callbacks, src != nullptr, src, node);
    _RYML_CB_ASSERT_VISIT(m_callbacks, node != NONE, src, node);
    _RYML_CB_ASSERT_VISIT(m_callbacks, parent != NONE, this, parent);
    _RYML_CB_ASSERT_VISIT(m_callbacks,  ! src->is_root(node), src, node);
...
id_type Tree::duplicate_children(Tree const* src, id_type node, id_type parent, id_type after)
{
    ...
    _RYML_CB_ASSERT_VISIT(m_callbacks, node != NONE, src, node);
    _RYML_CB_ASSERT_VISIT(m_callbacks, parent != NONE, this, parent);
    _RYML_CB_ASSERT_VISIT(m_callbacks, after == NONE || has_child(parent, after), this, parent);
...
void Tree::duplicate_contents(Tree const *src, id_type node, id_type where)
{
    _RYML_CB_ASSERT_VISIT(m_callbacks, src != nullptr, src, node);
    _RYML_CB_ASSERT_VISIT(m_callbacks, node != NONE, src, node);
    _RYML_CB_ASSERT_VISIT(m_callbacks, where != NONE, this, where);
...
id_type Tree::duplicate_children_no_rep(Tree const *src, id_type node, id_type parent, id_type after)
{
    ...
    if(after != NONE)
    {
        ...
        _RYML_CB_ASSERT_VISIT(m_callbacks, after_pos != NONE, this, node);
    }
    ...
    for(id_type i = src->first_child(node); i != NONE; i = src->next_sibling(i))
    {
        ...
        {
            _RYML_CB_ASSERT_VISIT(m_callbacks, is_map(parent), this, parent);
...
{
    _RYML_CB_ASSERT_VISIT(m_callbacks, src != nullptr, src, src_node);
    if(src_node == NONE)
        src_node = src->root_id();
    if(dst_node == NONE)
        dst_node = root_id();
    _RYML_CB_ASSERT_VISIT(m_callbacks, src->has_val(src_node) || src->is_seq(src_node) || src->is_map(src_node), src, src_node);
    if(src->has_val(src_node))
    {
        ...
        {
            _RYML_CB_ASSERT_VISIT(m_callbacks, src->is_val(src_node), src, src_node);
    ...
    {
        _RYML_CB_ASSERT_VISIT(m_callbacks, src->is_map(src_node), src, src_node);
        if( ! is_map(dst_node))
        {
            if(has_children(dst_node))
                remove_children(dst_node);
...
{
    _RYML_CB_ASSERT_VISIT(m_callbacks, node != NONE, this, node);
    _RYML_CB_ASSERT_VISIT(m_callbacks, is_map(node), this, node);
    if(get(node)->m_first_child == NONE)
    {
        _RYML_CB_ASSERT_VISIT(m_callbacks, _p(node)->m_last_child == NONE, this, node);
        return NONE;
    }
    else
    {
...
{
    _RYML_CB_ASSERT_VISIT(m_callbacks,  ! has_children(node), this, node);
    _RYML_CB_ASSERT_VISIT(m_callbacks, parent(node) == NONE || ! parent_is_map(node), this, node);
...
{
    _RYML_CB_ASSERT_VISIT(m_callbacks,  ! has_children(node), this, node);
    _RYML_CB_ASSERT_VISIT(m_callbacks, parent(node) == NONE || ! parent_is_map(node), this, node); // parent must not have children with keys
...
{
    _RYML_CB_ASSERT_VISIT(m_callbacks,  ! has_children(node), this, node);
    _RYML_CB_ASSERT_VISIT(m_callbacks, parent(node) == NONE || parent_is_seq(node), this, node);
...
{
    _RYML_CB_CHECK_BASIC(m_callbacks, !td.handle.empty());
    _RYML_CB_CHECK_BASIC(m_callbacks, !td.prefix.empty());
    _RYML_CB_CHECK_BASIC(m_callbacks, td.handle.begins_with('!'));
    _RYML_CB_CHECK_BASIC(m_callbacks, td.handle.ends_with('!'));
    // https://yaml.org/spec/1.2.2/#rule-ns-word-char
    _RYML_CB_CHECK_BASIC(m_callbacks, td.handle == '!' || td.handle == "!!" || td.handle.trim('!').first_not_of("01234567890abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ-") == npos);
    id_type pos = num_tag_directives();
    _RYML_CB_CHECK_BASIC(m_callbacks, pos < RYML_MAX_TAG_DIRECTIVES);
...
size_t Tree::resolve_tag(substr output, csubstr tag, id_type node_id) const
{
    ...
    if(tag.begins_with('!'))
    {
        ...
        {
            _RYML_CB_ERR_VISIT(m_callbacks, this, node_id, "tag directive not found");
        }
    }
...
csubstr _transform_tag(Tree *t, csubstr tag, id_type node)
{
    ...
    _RYML_CB_ASSERT_VISIT(t->m_callbacks, t->arena().str == prev_arena, t, node);
    size_t actual_size = t->resolve_tag(buf, tag, node);
    _RYML_CB_ASSERT_VISIT(t->m_callbacks, actual_size <= required_size, t, node);
...
id_type Tree::_next_node(lookup_result * r, _lookup_path_token *parent) const
{
    ...
    {
        _RYML_CB_ASSERT_VISIT(m_callbacks, !token.value.begins_with('['), this, r->closest);
        //_RYML_CB_ASSERT_VISIT(m_callbacks, is_container(r->closest) || r->closest == NONE);
        _RYML_CB_ASSERT_VISIT(m_callbacks, is_map(r->closest), this, r->closest);
        node = find_child(r->closest, token.value);
    }
    else if(token.type == KEYVAL)
    {
        _RYML_CB_ASSERT_VISIT(m_callbacks, r->unresolved().empty(), this, r->closest);
        if(is_map(r->closest))
            node = find_child(r->closest, token.value);
    }
    else if(token.type == KEY)
    {
        _RYML_CB_ASSERT_VISIT(m_callbacks, token.value.begins_with('[') && token.value.ends_with(']'), this, r->closest);
        token.value = token.value.offs(1, 1).trim(' ');
        id_type idx = 0;
        _RYML_CB_CHECK_BASIC(m_callbacks, from_chars(token.value, &idx));
...
id_type Tree::_next_node_modify(lookup_result * r, _lookup_path_token *parent)
{
    ...
    if(token.type == MAP || token.type == SEQ)
    {
        ...
        else
        {
            ...
            {
                id_type pos = NONE;
                _RYML_CB_CHECK_BASIC(m_callbacks, c4::atox(token.value, &pos));
                _RYML_CB_ASSERT_VISIT(m_callbacks, pos != NONE, this, r->closest);
    ...
    else if(token.type == KEYVAL)
    {
        ...
        {
            _RYML_CB_ASSERT_VISIT(m_callbacks, !is_seq(r->closest), this, r->closest);
    ...
    else if(token.type == KEY)
    {
        ...
        _RYML_CB_ASSERT_VISIT(m_callbacks, is_container(r->closest), this, r->closest);
        node = child(r->closest, idx);
        if(node == NONE)
        {
            _RYML_CB_ASSERT_VISIT(m_callbacks, num_children(r->closest) <= idx, this, r->closest);
    ...
    _RYML_CB_ASSERT_VISIT(m_callbacks, node != NONE, this, r->closest);
    *parent = token;
    return node;
}
...
Tree::_lookup_path_token Tree::_next_token(lookup_result *r, _lookup_path_token const& parent) const
{
    ...
    _RYML_CB_ASSERT_VISIT(m_callbacks, unres[pos] == '.' || unres[pos] == '[', this, r->closest);
    if(unres[pos] == '.')
    {
        _RYML_CB_ASSERT_VISIT(m_callbacks, pos != 0, this, r->closest);
        _advance(r, pos + 1);
        return {unres.first(pos), MAP};
    }

    _RYML_CB_ASSERT_VISIT(m_callbacks, unres[pos] == '[', this, r->closest);
    _advance(r, pos);
