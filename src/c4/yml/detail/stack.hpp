...
public:
    ...
    {
        _RYML_CB_ASSERT_BASIC(m_callbacks, (const char*)&n + sizeof(T) < (const char*)m_stack || &n > m_stack + m_capacity);
    ...
    void push_top()
    {
        _RYML_CB_ASSERT_BASIC(m_callbacks, m_size > 0);
        if(m_size == m_capacity)
        {
            id_type cap = m_capacity == 0 ? N : 2 * m_capacity;
            reserve(cap);
    ...
    C4_ALWAYS_INLINE T const& C4_RESTRICT top() const { _RYML_CB_ASSERT_BASIC(m_callbacks, m_size > 0); return m_stack[m_size - 1]; }
    C4_ALWAYS_INLINE T      & C4_RESTRICT top()       { _RYML_CB_ASSERT_BASIC(m_callbacks, m_size > 0); return m_stack[m_size - 1]; }

    C4_ALWAYS_INLINE T const& C4_RESTRICT bottom() const { _RYML_CB_ASSERT_BASIC(m_callbacks, m_size > 0); return m_stack[0]; }
    C4_ALWAYS_INLINE T      & C4_RESTRICT bottom()       { _RYML_CB_ASSERT_BASIC(m_callbacks, m_size > 0); return m_stack[0]; }

    C4_ALWAYS_INLINE T const& C4_RESTRICT top(id_type i) const { _RYML_CB_ASSERT_BASIC(m_callbacks, i < m_size); return m_stack[m_size - 1 - i]; }
    C4_ALWAYS_INLINE T      & C4_RESTRICT top(id_type i)       { _RYML_CB_ASSERT_BASIC(m_callbacks, i < m_size); return m_stack[m_size - 1 - i]; }

    C4_ALWAYS_INLINE T const& C4_RESTRICT bottom(id_type i) const { _RYML_CB_ASSERT_BASIC(m_callbacks, i < m_size); return m_stack[i]; }
    C4_ALWAYS_INLINE T      & C4_RESTRICT bottom(id_type i)       { _RYML_CB_ASSERT_BASIC(m_callbacks, i < m_size); return m_stack[i]; }

    C4_ALWAYS_INLINE T const& C4_RESTRICT operator[](id_type i) const { _RYML_CB_ASSERT_BASIC(m_callbacks, i < m_size); return m_stack[i]; }
    C4_ALWAYS_INLINE T      & C4_RESTRICT operator[](id_type i)       { _RYML_CB_ASSERT_BASIC(m_callbacks, i < m_size); return m_stack[i]; }
...
void stack<T, N>::reserve(id_type sz)
{
    ...
    _RYML_CB_ASSERT_BASIC(m_callbacks, ((uintptr_t)buf % alignof(T)) == 0u);
...
{
    _RYML_CB_ASSERT_BASIC(m_callbacks, m_stack != nullptr); // this structure cannot be memset() to zero
    if(m_stack != m_buf)
    {
        m_callbacks.m_free(m_stack, (size_t)m_capacity * sizeof(T), m_callbacks.m_user_data);
        m_stack = m_buf;
    ...
    {
        _RYML_CB_ASSERT_BASIC(m_callbacks, m_capacity == N);
    }
}
...
void stack<T, N>::_cp(stack const* C4_RESTRICT that)
{
    ...
    {
        _RYML_CB_ASSERT_BASIC(m_callbacks, that->m_capacity > N);
        _RYML_CB_ASSERT_BASIC(m_callbacks, that->m_size <= that->m_capacity);
    }
    else
    {
        _RYML_CB_ASSERT_BASIC(m_callbacks, that->m_capacity <= N);
...
void stack<T, N>::_mv(stack * that)
{
    ...
    _RYML_CB_ASSERT_BASIC(m_callbacks, that->m_stack != m_buf);
