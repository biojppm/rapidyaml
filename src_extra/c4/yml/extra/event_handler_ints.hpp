    ...
    void actually_val_is_first_key_of_new_map_block()
    {
        if(m_evt_prev < m_evt_size)
        {
            ...
            if(pos >= 0)
            {
                ...
                _RYML_ASSERT_BASIC_(m_stack.m_callbacks, (m_evt[pos] & ievt::BDOC));
                if(m_evt_pos < m_evt_size)
                {
                    ++pos; // add 1 to write after BDOC
                    int32_t num_move = m_evt_pos - pos;
                    int32_t posp1 = pos + 1;
                    _RYML_ASSERT_BASIC_(m_stack.m_callbacks, (m_evt[pos] & (ievt::BSEQ|ievt::BMAP)));
                    _RYML_ASSERT_BASIC_(m_stack.m_callbacks, num_move > 0);
...
public:
    ...
    int32_t _find_matching_open(ievt::DataType open, ievt::DataType close, int32_t pos) const
    {
        ...
        _RYML_ASSERT_BASIC_(m_stack.m_callbacks, (m_evt[pos] & close));
        _RYML_ASSERT_BASIC_(m_stack.m_callbacks, !(m_evt[pos] & open));
