...
inline c4::EnumSymbols<yml::extra::ievt::EventFlags> const esyms<yml::extra::ievt::EventFlags>()
{
    ...
    static constexpr const c4::EnumSymbols<EventFlags>::Sym syms[] = {
        {KEY_, "KEY_"},
        {VAL_, "VAL_"},
        {SCLR, "SCLR"},
        {BSEQ, "BSEQ"},
...
void print_events_ints(csubstr parsed_yaml, ievt::DataType const* evts_ints, ievt::DataType evts_ints_sz);
...
public:
    ...
    void actually_val_is_first_key_of_new_map_flow()
    {
        ...
        if(m_evt_prev + 1 < m_evt_size)
        {
            if(m_evt[m_evt_prev] & ievt::WSTR)
            {
                ...
                {
                    // watchout: it must done be in this order!
                    if(m_evt_curr + 1 < m_evt_size)
                        m_evt[m_evt_curr + 1] = m_evt[m_evt_curr];
                ...
                // move PSTR to prev
                if(m_evt[m_evt_prev + 1] & ievt::PSTR)
                {
                    m_evt[m_evt_prev] |= ievt::PSTR;
                    m_evt[m_evt_prev + 1] &= ~ievt::PSTR;
                }
            }
            else
            {
    ...
    int32_t _find_matching_open(ievt::DataType open, ievt::DataType close, int32_t pos) const
    {
        ...
        pos -= (m_evt[pos] & ievt::PSTR) ? 3 : 1; // don't count the starting close token
        uint32_t count = 0;
        while(pos >= 0)
        {
            ievt::DataType e = m_evt[pos];
            _c4dbgpf("find_matching: pos={} count={} e={}", pos, count, ievt::mkstring(m_evt[pos], m_evtstrbuf0));
            if(e & close)
            {
                _c4dbgpf(".............: pos={} close! count={} e={}", pos, count, ievt::mkstring(m_evt[pos], m_evtstrbuf0));
                ++count;
            }
            else if(e & open)
            {
                _c4dbgpf(".............: pos={} open! count={} e={}", pos, count, ievt::mkstring(m_evt[pos], m_evtstrbuf0));
                if(!count)
                    return pos;
                else
                    --count;
